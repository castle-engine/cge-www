...
castle_thumbs(array(
  ...
  <li>
    <p>There\'s a new demo in engine <a href="https://castle-engine.sourceforge.io/kambi_vrml_game_engine.php#section_svn">SVN sources</a>: <code>terrain</code> (look inside <code>kambi_vrml_game_engine/examples/vrml/terrain/</code> directory). It shows basic procedural terrain generation. Uses cosine interpolated noise, summed by <i>Fractional Brownian Motion</i> (which is a fancy way of saying "sum a few scaled noise functions"&nbsp;:)&nbsp;).</p>

    <p>It can also load a terrain data from <a href="http://www2.jpl.nasa.gov/srtm/">SRTM (.hgt files)</a> (for example, <a href="http://netgis.geo.uw.edu.pl/srtm/Europe/">sample files for Europe are here</a>). And it can display a terrain defined by mathematical expression, like <i>sin(x*10) * sin(y*10)</i> (see <a href="https://castle-engine.sourceforge.io/kambi_script.php">KambiScript language reference</a> for full syntax and functions available for math expressions).</p>
  ...
  <li>
    ...
    <ol>
      ...
      <li>Also, new kambi_vrml_game_engine*.tar.gz archives will contain offline HTML docs by pasdoc (the same ones <a href="https://castle-engine.sourceforge.io/apidoc/html/index.html">as available online</a>).
    </ol>
...
<ul>
  ...
  <li><p>New <a href="http://wdune.ourproject.org/">White Dune</a> release supports all <a href="https://castle-engine.sourceforge.io/x3d_extensions.php">VRML/X3D extensions</a> of our engine. Thanks go to Joerg "MUFTI" Scheurich.</p></li>
...
<ul>
  <li><p>All X3D multi-texturing nodes implemented. See also <a href="https://castle-engine.sourceforge.io/x3d_implementation_texturing.php#section_multi_texturing_clarifications">clarifications how MultiTexture.mode/source fields work and how to separate them for rgb and alpha channel</a>.</p></li>

  <li><p>All X3D cube map nodes implemented. This includes <code>GeneratedCubeMapTexture</code>, very useful to make mirrors, especially with the help of <a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_tex_coord_worldspace">WORLDSPACEREFLECTIONVECTOR extensions</a>.</p></li>

  <li><p>All X3D 3D texture nodes implemented.</p></li>

  <li><p>DDS (DirectDraw Surface) format is supported, for all texture types (2D, 3D in <code>ImageTexture3D</code>, cube map in <code>ImageCubeMapTexture</code>). S3TC compression, explicit mipmaps are all supported, <a href="https://castle-engine.sourceforge.io/x3d_implementation_texturing.php#section_dds">more details here</a>. New ' . news_a_href_page('glViewImage 1.3.0', 'glviewimage') . ' supports reading, writing and even limited editing of DDS images.<!-- Many other usability fixes were done to <code>glViewImage</code> along the road.--></p></li>

  <li><p><a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_rendered_texture">RenderedTexture</a> node is implemented: a texture rendered from a specified viewpoint.</p></li>

  <li><p>Passing to GLSL shaders various uniform value types is implemented. This includes vectors, matrices and many more. <a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_viewpoint_camera_matrix">Viewpoint.camera*Matrix</a> may be very useful to feed to shaders. You can also <a href="https://castle-engine.sourceforge.io/x3d_implementation_shaders.php#glsl_passing_uniform_textures">pass texture nodes to GLSL shader uniforms, following X3D specification</a>.</p></li>

  <li><p>New extensions to easily make <a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_shadow_maps">projective texturing and shadow maps</a> within your VRML/X3D worlds.</p></li>
...
<ul>
  <li><p>When using single texturing, you can set environment mode to replace <a href="https://castle-engine.sourceforge.io/x3d_implementation_status.php#default_texture_mode_modulate">(default is modulate)</a>.</p></li>

  <li><p><a href="https://castle-engine.sourceforge.io/kambi_script.php">KambiScript</a> functions to operate on string characters: <code>"character_from_code"</code>, overloaded <code>"array_set", "array_get", "array_get_count", "array_set_count"</code> for strings.</li>

  <li><p>As usual, along with view3dscene release, we also release <a href="https://castle-engine.sourceforge.io/kambi_vrml_game_engine.php">accompanying Kambi VRML engine (version 1.8.0)</a> for developers. Released binaries are compiled with FPC 2.2.4, sources can also be compiled with FPC from trunk (tested on 2009-08-21). I also provide binaries for Linux/x86_64 (not only 32-bit Linux/i386), as I see a demand for it.</li>
</ul>'
    ),
...
<ul>
  <li><a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_rendered_texture">RenderedTexture</a> node is implemented: a texture rendered from a specified viewpoint. Useful for many effects. The most straightforward use would be to make a "security camera" or a "portal", through which a player can peek what happens at the other place in 3D world. (<a href="https://castle-engine.sourceforge.io/miscella/rendered_texture_one_file.x3dv">Simple example</a>).</li>
...
<ul>
  ...
  <li>New extensions to easily make
    <a href="https://castle-engine.sourceforge.io/x3d_extensions_shadow_maps.php">projective
...
<ul>
  ...
  <li><a href=\"https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_octree_properties\">KambiOctreeProperties, an extensions to specify octree limits for your scene</a>. <a href=\"" . CURRENT_URL . "vrml_engine_doc/output/xsl/html/section.octrees_dynamic.html\">Section \"Octrees for dynamic worlds\" added to the documentation</a>, to explain how octree works since 1.6.0 version. The shape octree was speed up by mailboxes.</li>
  ...
  <li>The path tracer honors VRML &gt;= 2.0 materials, and <a href=\"https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_material_phong_brdf_fields\">VRML &gt;= 2.0 materials have the physical fields</a>. Because of this, also " . news_a_href_page('rayhunter 1.3.0', 'rayhunter') . " is released.</li>
</ul>
