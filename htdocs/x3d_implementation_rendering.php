...
<p>Most of the nodes in this section have both indexed (like <?php echo cgeRef('TIndexedTriangleSetNode'); ?>) and non-indexed (like <?php echo cgeRef('TTriangleSetNode'); ?>) versions.

<p>The <i>"indexed"</i> versions allow to refer to the same vertex (on the <code>Coord</code> list) multiple times, since you specify the vertex once in <?php echo cgeRef('TCoordinateNode'); ?>, and then can use the same index multiple times e.g. in the array you pass to <?php echo cgeRef('TIndexedTriangleSetNode.SetIndex'); ?>.

<p>If you have a large mesh that has vertexes shared by neighboring polygons, it usually makes sense to use "indexed" versions. They will be slightly more optimal &mdash; the vertex is specified once, some calculations can run on it just once (even if it is then used by multiple polygons). This is often a natural representation of a triangle set when generated by 3D authoring tools like <a href="blender">Blender</a>.

<p>Otherwise, for smaller meshes, and meshes that would not reuse the vertexes anyway (because they specify mostly unconnected triangles), you can use non-indexed versions.

<p>Most of geometries created <i>"by code"</i> usually qualify as "smaller meshes" :) So just use whatever is most natural, which is usually non-indexed versions. It probably doesn't make sense to even think about optimizing when vertex count &lt; 100.
