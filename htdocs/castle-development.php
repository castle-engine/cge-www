<?php
  ...
  $toc = new TableOfContents(
    array(
      ...
      new TocItem('General considerations', '3d_general', 1),
...
sending simple feedback is definitely useful too.
...
<ul>
  <li><p>Finish fixing Radeon issues.

  <li><p>For now there are minor issues with designing castle levels
    in VRML 2.0. While my engine is fully capable to handle VRML 2.0
    files generated by Blender exporter, some minor things in castle-specific
    units were not updated correctly yet. For details see source code, file
    <tt>source/kambi_vrml_game_engine/3dmodels/TODO.vrml97</tt>.

    <p>In other words: if you want to be on the safe side, don't
    use VRML 2.0 models for game's data for now. Use VRML 1.0 instead.
    This should be fixed in soon.
    Some hints below in this file are specific for VRML 1.0.
...
this in my game licensed on GNU GPL. For a list of various sites
...
Including good export for VRML 1.0 and 2.0, of course. Basically there are no special
rules for designing in Blender models for my engine &mdash;
...
<ul>
  ...
  <li><p>When loading level, we search for node named <tt>LevelBox</tt>.
    Such node should be a parent of some shape (e.g. <tt>IndexedFaceSet</tt>)
    node. (In other words: in Blender just add a mesh (usually a cube)
    and name it <tt>LevelBox</tt>).
...
<ul>
  ...
  <li><p>For testing various animations of creatures (and adjusting their time
    ...
    (see <tt>kambi_vrml_game_engine/3dmodels.gl/examples/demo_animation.dpr</tt>).
