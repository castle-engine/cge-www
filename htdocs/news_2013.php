...
<ol>
  ...
  <li><p>A lot of engine API improvements done in the recent weeks, in particular around 2D controls and images. Most of that is caused by the desire to port complete engine to OpenGL ES, and have Android/iOS version.
    ...
    <p><b><code>TGLImage</code> (our class to render images as 2D, for GUI and 2D games) much improved</b>:
    <ul>
      <li>Renders 2D images as npot textures underneath (suitable for modern OpenGL and GLES, better with anti-aliasing).
      <li><code>TGLImage.Draw</code> can display image stretched (if ScalingPossible, OpenGL bilinear filtering will stretch it nicely).
      <li><code>TGLImage.Draw3x3</code> can display image stretched intelligently, preserving corners and sides. These functions are the basis for implementing 2D GUI, e.g. <code>TGLImage.Draw3x3</code> is used for all <code>TCastleButton</code> states, which makes it also easier to theme (e.g. it\'s now possible to make rounded corners without any fuss, just make transparent corners in the texture). All <code>CastleControls</code> are now drawn using <code>TGLImage</code>.
      <li><code>TGLImage</code> drawing automatically uses alpha test or alpha blending, depending on alpha channel in the loaded image. You can always change <code>TGLImage.Alpha</code> to explicitly force specific alpha treatment. You can also change <code>TCastleImageControl.AlphaChannel</code>.
    ...
    <ul>
      <li><code>TCastleLabel</code> (like <code>Font.PrintStringsBox</code>, but nicely wrapped in a class).
      <li><code>TCastleDialog</code> (to make modal or non-modal dialog boxes, with scrollbars, buttons etc).
      <li><code>TCastleProgressBar</code> (display progress bar as <code>TUIControl</code>, regardless if you use <code>CastleWindow</code> or <code>CastleControl</code>).
    </ul>
    ...
    <ul>
      <li>Always use <code>SetWindowPos</code>, never directly use <code>glRasterPos*</code> functions. In general, forget about raster position &mdash; this concept is gone in GLES2 and modern OpenGL. Use only <code>SetWindowPos</code> to affect initial text position for <code>TGLBitmapFont</code> and <code>TGLImage</code>, this works in both engine 4.1.1 and in SVN (future 4.2.0). It is only for 2D, and is not affected by modelview matrix state.
      <li>Since 4.2.0, it will be advised to use cleaner <code>TGLBitmapFont.Print(X,Y,Color,string)</code> and <code>TGLImage.Draw(X,Y)</code> instead of <code>SetWindowPos</code>. But <code>SetWindowPos</code> will also work, to allow you to write code in the existing 4.1.1 API that will also work smoothly in engine &gt;= 4.2.0.
      <li>If you hope to keep your code portable to GLES2, of course be sure to avoid old OpenGL API. Immediate mode rendering cannot be used (use only VBO; usually, you should just use our <code>TCastleScene</code> for rendering all 3D, and <code>TGLImage</code> or <code>TGLVideo2D</code> for all 2D). <!--Push/pop of matrices and attributes is also not available.-->
    </ul>

    <p><b>Font API improved</b>:
    <ul>
      <li>The advised font drawing call is now <code>Print(X,Y,Color)</code>. Other versions are deprecated, as they lead to messy code &mdash; it\'s not nice to manage global state in <code>WindowPos</code>, it\'s not nice to manage global color in <code>CurrentColor</code>.
      <li>All font drawing can use blending for text (just pass text color with alpha < 1).
    </ul>

    <p><b>Consistent <code>TCastleColor</code> usage</b>. We now consistently use <code>TVector4Single</code> type to express a color, it\'s even aliased as <code>TCastleColor</code>.
    <ul>
      <li>We do not use TVector3Single (as we like having alpha available).
      <li>We do not use byte versions. There are numerous arguments in favor of using float-based color values (instead of byte-based): it allows for components &gt; 1 (useful for physically-correct rendering and some shaders inputs), it is natural to smoothly interpolate (you can also use functions in <code>CastleColors</code> to interpolate in HSV space), it is easier to express constants.

        <p>The only advantage of using byte-based colors is that this is what you get from RGB 32-bit color images, and you can compare it for exact equality. But this isn\'t universal anyway (there are images with float colors, like RGBE). When dealing with <code>TRGBImage</code> or <code>TRGBAlphaImage</code> you can still use byte-based colors and eventually convert using Vector4Single() and Vector4Byte() both ways (in practice 8-bit and 16-bit values will be expressed precisely as Single too).
...
<ol>
  ...
  <li><code>MultiTexture.function</code> support. <a href="http://svn.code.sf.net/p/castle-engine/code/trunk/demo_models/multi_texturing/functions.x3dv">Demo model</a>.
  <li><code>NavigationInfo.transitionComplete</code> support. <a href="http://svn.code.sf.net/p/castle-engine/code/trunk/demo_models/navigation/transition_multiple_viewpoints.x3dv">Demo model transition_multiple_viewpoints.x3dv</a> shows how to use it to make an animated transition between a couple of viewpoints.
  <li>Support for 8 and 16 samples for anti-aliasing.
  <li>If you load or save image sequences using the syntax <code>image%d.png</code>, for example inside our extension <a href="' . CURRENT_URL . 'x3d_extensions.php#section_ext_movie_from_image_sequence">Movies for MovieTexture can be loaded from images sequence</a>: the new syntax to indicate counter inside the URL is <code>@counter(4)</code>, where 4 is the padding. For example <code>image%d.png</code> has to be changed to <code>image@counter(1).png</code> and <code>image%4d.png</code> has to be changed to <code>image@counter(4).png</code>. See previous news for explanation why this change is necessary.
  <li>FullScreen switching much improved: you can now freely change <a href="' . CURRENT_URL . 'apidoc/html/CastleWindow.TCastleWindow.html#FullScreen">TCastleWindow.FullScreen</a> property at runtime. For backends that handle it (GTK, LCL) the switch may happen by resizing the window, instead of recreating it, which means it avoids (sometimes time-consuming) reinitialization of OpenGL resources. On Mac OS X, fullscreen mode hides the dock and menu, so they don\'t cover the window. Added <a href="' . CURRENT_URL . 'apidoc/html/CastleWindow.TMenuItemToggleFullScreen.html">TMenuItemToggleFullScreen</a> for comfort.
  <li>You can now load <a href="' . CURRENT_URL . 'creating_data_material_properties.php">material_properties.xml files</a> in view3dscene, will be used to enhance all subsequent materials. You can also specify alpha_channel (see <a href="' . CURRENT_URL . 'x3d_extensions.php#section_ext_alpha_channel_detection">alphaChannel extension</a>) in material properties. I expect to enhance this in the next release, so that you can add there also stuff like <code>TextureProperties</code> (for anisotropic filtering and more, right now you have to edit VRML/X3D to add it, which is not always comfortable when exporting VRML/X3D e.g. from Blender).
</ol>
...
<ol>
  ...
  <li><a href="' . CURRENT_URL . 'apidoc/html/CastleFilesUtils.html#ApplicationData">ApplicationData</a> and <a href="' . CURRENT_URL . 'apidoc/html/CastleFilesUtils.html#ApplicationConfig">ApplicationConfig</a> to get data/config dirs as URLs. On Mac OS X <code>ApplicationData</code> can use data from bundle <code>Contents/Resources/data</code> . See also useful <code>CastleFilesUtils.BundlePath</code> function.
  <li>Nicer <a href="' . CURRENT_URL . 'apidoc/html/CastleUIControls.TInputListener.html#Update">TInputListener.Update</a> API, with simple HandleInput parameter. It used to be called <code>Idle</code>, but we renamed it to <code>Update</code>, since this describes its behavior correctly.
...
<ol>
  <li><p><code>NavigationInfo.transitionComplete</code> support. <a href="http://svn.code.sf.net/p/castle-engine/code/trunk/demo_models/navigation/transition_multiple_viewpoints.x3dv">Demo model transition_multiple_viewpoints.x3dv</a> shows how to use it to make an animated transition between a couple of viewpoints.
  ...
  <li><p><code>MultiTexture.function</code> support (forces shader pipeline rendering for given shape; there\'s no way to reasonably implement this using fixed-function pipeline). <a href="http://svn.code.sf.net/p/castle-engine/code/trunk/demo_models/multi_texturing/functions.x3dv">Demo in functions.x3dv</a>.
  ...
  <li><p>If you load or save image sequences using the syntax <code>image%d.png</code>, for example inside our extension <a href="' . CURRENT_URL . 'x3d_extensions.php#section_ext_movie_from_image_sequence">Movies for MovieTexture can be loaded from images sequence</a>: the new syntax to indicate counter inside the URL will be <code>@counter(4)</code>, where 4 is the padding. For example <code>image%d.png</code> has to be changed to <code>image@counter(1).png</code> and <code>image%4d.png</code> has to be changed to <code>image@counter(4).png</code>.

    <p>For loading, you will <i>have</i> to use new syntax with <code>@counter(&lt;padding&gt;)</code> with new view3dscene / Castle Game Engine versions. You will have to update your VRML/X3D models, old syntax will unfortunately not work anymore (reasons below). For saving, the old syntax <code>%d</code> will continue to work for some time (along the new <code>@counter(&lt;padding&gt;)</code> syntax, and you\'re encouraged to upgrade to new syntax).

    <p>The reason for this is that <code>MovieTexture.url</code> is now correctly treated as an URL, and this means that percent character <code>%</code> needs to be escaped to <code>%25</code>. Inside URL the sequence <code>%4d</code> has to mean letter <code>M</code> (ASCII code 77, which is 4d in hexadecimal). So there is unfortunately no way to avoid breaking compatibility &mdash; we want to correctly support URLs, which implies that <code>%4d</code> must be interpreted as letter "M", not as a magic counter.
...
<ol>
  ...
  <li><p><b>Network (http) support.</b> Most "FileName" parameters and properties are now URLs. You can use protocols like <code>file:</code> and <code>http:</code> and <code>data:</code> everywhere, and of course http will be automatically downloaded. Try <a href="http://michalis.ii.uni.wroc.pl/castle-engine-snapshots/">view3dscene from snapshots</a>, enable <i>"Preferences-&gt;Download Resources From Network"</i> and then use menu item (Ctrl+L) to load URL, or pass URL on the command-line.</p>
    ...
    <p>In a game, you could also use URLs inside files like <code>level.xml</code> and <code>resource.xml</code>. For example, you could distribute now a level.xml file that contains URLs to levels on your server, downloaded each time.

  <li><p><b>Clipboard</b> (Ctrl+C, Ctrl+V, Ctrl+X in message boxes, especially handy to copy/paste URLs). For developers: use <code>Clipboard.AsText</code> property. Implemented for CastleWindow WinAPI, GTK, LCL backends.
...
<ol>
  <li><p>We introduce a <b>high-level game API, to construct a fully-working 3D game really easy</b>. This includes integrated units to manage levels, creatures (with smart AI), items, player, with inventory, management of 3D resources, easy 3D sound, flexible game data layout (you can use XML files to define a lot of things without touching game code), and so on.</p>
    ...
    <p>Of course, we use X3D for all the 3D models, so everything can be animated and interactive, and may use features like shadows, mirrors, shaders and such. See <a href="' . CURRENT_URL . 'demo_models.php">our demo 3D models</a> (in particular, check new <code>water/caustics/</code> demo inside, by Victor Amat, <a href="http://youtu.be/1mUU8prDi9k">movie showing it is here</a>).</p>

    <p>There are a couple new examples in engine sources. First of all, <code>examples/fps_game/</code> is a must-see: it shows a complete working FPS game, with a lot of comments in the source code and data files.</p>
    ...
    <!--
    <p>Another new example program is the <code>examples/resource_animations</code> to view creature/item animations. This is accompanied by <a href="' . CURRENT_URL . 'creating_data_resources.php">creating data guide about resources</a>, that explains new methods to define 3D animations.</p>
    -->
  </li>
...
<ol>
  <li>
    <p><b>New very important example program is available in engine source code: <code>fps_game</code></b>.</p>
  ...
  <li><p><b>Improvements to game logic:</b></p>
    <ul>
      <li>Creature enemy is now configurable by overriding <code>TWalkAttackCreature.Enemy</code> method. The default implementation chooses the central player as the enemy (if the player is not dead yet), but you can override it to implement something more sophisticated (like creature vs creature fighting, teams etc. &mdash; not everything is tested there yet, but it all should be doable now, so go wild :).</li>
  ...
  <li><p><b>Final code renames/refactoring:</b></p>
    <ul>
      <li>A lot of unit renames, to bring all units to <code>CastleXxx</code> names. I admit it was a mistake trying to have non-prefixed unit names in engine 3.0.0. After some time, many unit names had to be prefixed anyway, and this created inconsistent naming &mdash; some unit names had <code>Castle</code> prefix, some not. Now all unit names have <code>Castle</code> prefix (the only exception being <code>X3DXxx</code> units).
      ...
      <li>Many <code>DefaultXxx</code> global constants moved to constants inside classes, to not pollute namespaces with a lot of constants that are usually useful only as default value for corresponding property Xxx.</li>
      <li>All creature/items "kinds" renamed to "resources". Previously, terms "kinds" and "resources" were used to refer to the same thing, which could create confusion. Now it\'s simple: <code>resource.xml</code> file refers to the resource class (maybe it\'s creature resource, maybe item resource and so on), and creates resource instance. <code>TCreatureResource</code> is derived from <code>T3DResource</code> (as <code>TCreature</code> derives from <code>T3D</code>). <code>TItemResource</code> is derived from <code>T3DResource</code>, like <code>TItemOnLevel</code> derives from <code>T3D</code>.</li>
      <li><code>CastleKeysMouse</code> (formerly <code>KeysMouse</code>) types are no longer aliased in <code>CastleWindow</code> unit. This was unmaintainable, and wasn\'t available in Lazarus <code>CastleControl</code>. Simply add <code>CastleKeysMouse</code> to your uses clause if you use any identifier from it.</li>
      <li>Removed units: <code>VideosCache</code> (merged into <code>CastleVideos</code>), <code>Rectangles</code> (merged to <code>CastleOnScreenMenu</code>), <code>BezierCurve</code> (merged to <code>CastleCurves</code>), <code>NormalizationCubeMap</code> (moved to <code>bump_mapping</code> example dir, not useful for engine; and <code>bump_mapping</code> was actually removed from engine core, as not useful anymore, it\'s now in <a href="http://michalis.ii.uni.wroc.pl/wsvn/michalis/obscure_castle_engine_demos_and_tools/">obscure_castle_engine_demos_and_tools repo</a>).</li>
    </ul>
  </li>

  <li><p><b>Other engine improvements:</b></p>
    <ul>
      <li><code>CastleImages</code> supports resizing images with bilinear interpolation. <code>TCastleImage.Resize</code> and <code>TCastleImage.MakeResized</code> take <code>Interpolation</code> parameter, <code>riNearest</code> or <code>riBilinear</code>. Default is still <code>riNearest</code>, so fast (and ugly) like before. <code>CastleWindowProgress</code>, when it needs to scale progress background image, requests <code>riBilinear</code> interpolation. <a href="' . CURRENT_URL . 'glviewimage.php">glViewImage</a> allows to test both resizing methods.</li>
      <li>Fonts: Huge refactoring of font units and types.
        <ul>
          <li>We now have 4 units: <code>CastleOutlineFonts</code>, <code>CastleBitmapFonts</code>, <code>CastleGLOutlineFonts</code>, <code>CastleGLBitmapFonts</code>.
          <li><code>TBitmapFont</code> and <code>TOutlineFont</code> are classes, with Data field and some methods. This way utilities from <code>CastleOutlineFonts</code> become methods.
          <li><code>font2pascal</code> utility added, this is a very old Windows-only utility to convert fonts into our bitmap/outline font structures. It awaits eagerly to be reimplemented using freetype, contributions for this are most welcome!
        </ul>
      </li>
      <li>Memory optimization for models with many nodes (esp. using heavily prototypes), by about 10%, and another 10% possible if you define <code>CONSERVE_TRIANGLE_MEMORY</code> symbol.</li>
      <li>Improvements to <code>CastleWindowProgress</code>: <code>Opacity</code> property, configurable colors, using <code>UIFontSmall</code> when necessary, fixed to work with non-standard glViewport (like after typical TCastleViewport usage).</li>
      ...
      <li>Examine camera has <code>ExclusiveEvents</code> = true, consistent with other TUIControl.</li>
      <li><code>CastleLog</code> output stream is configurable.</li>
    </ul>
  </li>
