...
to display player life or inventory (the engine does manage the
player's inventory automatically for you, in TPlayer.Inventory;
however, it's not displayed automatically, since various games have
wildly different needs). You can do it by defining a new TUIControl
descendant, where you can draw anything you want in overridden Draw
method. You can use the ready UIFont instance to draw a 2D font on the
...
<p>Inside TGame2DControls.Draw you have the full knowledge about the world,
...
<ul>
  <li><p>SetWindowPos (CastleGLUtils unit) sets the raster position in screen coordinates.

  <li><p>You have ready UIFont and UIFontSmall instances of TGLBitmapFont that
...
UIFont.Print(Format(\'Player life: %f / %f\', [Player.Life, Player.MaxLife]));'); ?>
  ...
  <li><p>Every inventory item has already loaded image (defined in resource.xml),
...
begin
  ...
  <li><p>CastleGLUtils contains ready procedures to draw
...
'if Player.Dead then
  ...
  GLFadeRectangle(0, 0, ContainerWidth, ContainerHeight,
    ...
    <p>Note that Player.FadeOutIntensity will be 0 when there is no pain, which cooperates
  ...
  <li><p>CastleGLUtils, and many other modules, provide many other helpers.
</ul>

<p>See fps_game for a working and fully-documented demo of such TGame2DControls
...
<p>You can use any 2D engine controls like this, just add them to Window.Controls.
