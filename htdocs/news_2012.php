...
'<p>Sorry, still no new release... But, as usual, we have many new features to announce for the upcoming <a href="https://castle-engine.sourceforge.io/engine.php">Castle Game Engine</a> 4.0.0 :) For starters, a little movie with explosions fun:</p>
...
<ol>
  <li><p><b>Creatures/items</b> (commonly called "resources" in many places) <b>improvements</b>:</p>
    <ol>
      <li><p>New example program: <code>resource_animations</code>. Demonstrates how to define creature/item animations in <code>resource.xml</code> files, from <a href="https://castle-engine.sourceforge.io/kanim_format.php">KAnim</a> and/or X3D animations. Can be also used to preview animations from any <code>resource.xml</code> files, e.g. you can try it on <a href="https://castle-engine.sourceforge.io/castle.php">castle1</a> creatures and items. See DRAFT.modeling_tutorial.txt (will be moved to nicer HTML page later) for notes about how to design your creatures/items.</p>
  ...
  <li><p>Our <a href="https://castle-engine.sourceforge.io/blender.php"><b>Blender X3D exporter</b></a> updated for Blender 2.64a.</p>
    <p>New <a href="https://castle-engine.sourceforge.io/blender.php"><b>KAnim exporter</b> for Blender 2.64a</a> added. For the same reason as a couple of years ago (Blender standard X3D exporter still cannot export animations) we still need it.</p></li>

  <li><p><b>Levels improvements</b>:</p>
    <ol>
      <li><p>Placeholders are nicer. You can indicate in <code>level.xml</code> how to detect placeholder names in 3D level file, for example use <code>placeholders="blender"</code> to use object names set in Blender. You configure it now nicely to support any other modeler. Also, we now make clear that this detection dependent on modeler is <i>only</i> for <code>TGameSceneManager.LoadLevel</code> placeholders, nothing else. See <a href="https://castle-engine.sourceforge.io/creating_data_levels.php">level.xml and resource.xml files documentation</a>.</p>
        <p>You can register own callbacks, see <code>PlaceholdersNames</code> and docs of <code>TPlaceholderName</code>.</p>
      </li>
      <li>Magic "margin" parameter around sectors removed. We now just look at waypoint\'s bounding box. See <a href="https://castle-engine.sourceforge.io/creating_data_levels.php">notes about sectors/waypoints for user</a>, and see <code>TGameSceneManager.LoadLevel</code> for developer docs.</li>
      <li>Extracting direction (for initial direction that the creature is facing) from placeholder. See <code>placeholder_default_direction</code> in <a href="https://castle-engine.sourceforge.io/creating_data_levels.php">creating_data_levels docs</a>.
  ...
  <li><p><b>Image API improvements</b>:</p>
    <ol>
      <li>Cleanups around <code>AlphaChannel</code> detection, code simplified (and much shortened). <a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_alpha_channel_detection">Our alphaChannel extension is now available for all X3DTextureNode.</a></li>
...
'<p>Work on the new <a href="https://castle-engine.sourceforge.io/engine.php">Castle Game Engine</a> 4.0.0 version continues :) Many improvements for the programmers that want to develop their own games using our engine:</p>

<ol>
  <li><p>Many improvements to
    <a href="https://castle-engine.sourceforge.io/tutorial_intro.php">engine tutorial</a>,
    <a href="https://castle-engine.sourceforge.io/tutorial_classes_overview.php">classes diagram</a> and
    <a href="https://castle-engine.sourceforge.io/creating_data_intro.php">level.xml and resource.xml files documentation</a>.
...
'<p>The work on the next <a href="https://castle-engine.sourceforge.io/engine.php">Castle Game Engine</a> version continues :) Looking at the size of the improvements, next version will deserve the new major number 4.0.0 (even though last version was 3.0.0). We have almost finished new engine features planned for the next version, and we have a lot of new documentation. <!-- (as the point of next engine version is to be better documented for developers). -->

<ol>
  <li><p>Diagram (as a text file...) of the most important "Castle Game Engine" classes is available.</p>

    <p>I may try to squeeze this into a real image (maybe just the important boxes, and add the text on a normal HTML page). If you have experience with some diagram-drawing tools (like <a href="https://live.gnome.org/Dia/">Dia</a> or other open-source tool) I would appreciate contributions here (<a href="https://castle-engine.sourceforge.io/forum.php">send them through forum or e-mail to Michalis</a>).</p>
  </li>
...
<p>Other changes, noticeable in <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> or next <a href="https://castle-engine.sourceforge.io/castle.php">"The Castle"</a> data:</p>

<ol>
  <li>In <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> <i>"Help -&gt; Selected Object Information"</i>, show Blender object/mesh name.</li>
  ...
  <li>New <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> menu item <i>"Remove placeholder nodes from "Castle Game Engine" levels"</i>, a general way to remove some specially-named nodes (see <a href="https://castle-engine.sourceforge.io/creating_data_levels.php">castle development doc</a> for reference of most of them, see sources for all). Removed previous castle-process-3d-model.lpr, it was uncomfortable and didn\'t really prove useful.</li>
...
'<p>Hi! Here are the latest news from the development of our <a href="https://castle-engine.sourceforge.io/engine.php">engine</a> and <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a>:</p>
...
<ol>
  <li>
    ...
    <p>This model shows a German Pavillion in the Universal Expo of Barcelona of 1929. The video shows a quick walk around the model in <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a>, showing reflections and caustics visible above and under the water. We also show that all screen effects, like <i>Screen Space Ambient Occlusion</i>, work now with shadow volumes without problems :)</p>
  </li>

  <li>
    <p><b>Lights editor</b> is available in <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a>. Use the menu <i>"Edit -&gt; Lights Editor"</i> to bring it on. It allows to interactively adjust a lot of lights settings. The scene is not paused &mdash; you can still trigger animations, walk around and generally do everything while you tweak lights settings. Of course you can try it right now by using <a href="http://michalis.ii.uni.wroc.pl/castle-engine-snapshots/">view3dscene from snapshots</a>.</p>
  ...
  <li><p>For <b>shadow volumes to work, your 3D models must now be perfect 2-manifold</b>.
    ...
    to be perfect manifolds. Remember that you can <a href="https://castle-engine.sourceforge.io/x3d_extensions_shadow_maps.php#section_shadow_caster">mark some shapes as not shadow casters</a>
...
<ol>
  ...
  <li><p><a href="https://castle-engine.sourceforge.io/lets_take_a_walk.php">lets_take_a_walk, demo of 3D sounds inside our engine</a>, was refreshed (to use T3D classes) and added as another example to our engine sources (under <code>castle_game_engine/examples/3d_sound_game/</code>).</p></li>

  <li><p>Added comments about sponsoring, and links to <a href="http://www.fossfactory.org/">FOSS Factory</a> and <a href="http://gun.io/">Gun.io</a> to our <a href="https://castle-engine.sourceforge.io/donate.php">donations</a> page.</p></li>
  ...
  <li><p>Other small fixes/updates:</p>
    ...
    <ul>
      ...
      <li>RenderedTexture.viewpoint fixed to send <a href="https://castle-engine.sourceforge.io/x3d_extensions.php#section_ext_viewpoint_camera_matrix">camera*Matrix events</a>.</li>
...
'<p>Hi everyone! We have a <i>lot</i> of news about new features and improvements in our <a href="https://castle-engine.sourceforge.io/engine.php">engine</a> and <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a>. No new release yet, we just announce what is available in SVN (and can be tested by <a href="http://michalis.ii.uni.wroc.pl/castle-engine-snapshots/">downloading view3dscene from snapshots</a>). I really have to start making these news posts more often, otherwise they get ridiculously long, well.. as long as this one :)</p>
...
<ol>
  ...
  <li><p>Thanks to Jan Adamec, we have a lot of new features and improvements to navigation in Walk/Fly modes:</p>
    ...
    <ul>
      <li><p>We <b>support <a href="http://www.3dconnexion.com/">3D mouse devices</a></b> in all our tools (like <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> and all engine examples, games etc.). Only under Windows for now &mdash; contributions for other OSes are welcome, of course.</p>
  ...
  <li><p>Also thanks to Jan Adamec, we have <b>Screen Space Ambient Occlusion</b> implemented inside our engine. Available as comfortable <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> menu item "View -&gt; Screen Effects -&gt; Screen Space Ambient Occlusion" (for developers: <code>TCastleAbstractViewport.ScreenSpaceAmbientOcclusion</code>.) Works on arbitrary models out-of-the-box :) Uses <a href="https://castle-engine.sourceforge.io/x3d_extensions_screen_effects.php">our screen effects framework</a>.</p></li>

  <li><p><b>Screen effects cooperate now wit multi-sampling (anti-aliasing)</b>. Previously, any screen effect (like SSAO, or <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> effects, or effects in VRML/X3D files like <code>demo_models/screen_effects/</code> from <a href="https://castle-engine.sourceforge.io/demo_models.php">demo models</a>) was disabling anti-aliasing.</p>

    <p>Now we can use <a href="http://www.opengl.org/registry/specs/ARB/texture_multisample.txt">OpenGL ARB_texture_multisample</a> to have multi-sample textures used with screen effects. The GLSL code of every screen effect is linked with some helper <code>screen_xxx</code> functions. If you use them, your effects will work both with and without multi-sampling. The new functions are documented in <a href="http://michalis.ii.uni.wroc.pl/castle-engine-snapshots/docs/x3d_extensions_screen_effects.html">future Screen Effects docs</a> (this will replace <a href="https://castle-engine.sourceforge.io/x3d_extensions_screen_effects.php">the current Screen Effects docs</a> at the next release). All existing engine effects, like SSAO and other view3dscene effects in "View-&gt;Screen Effects", are already ported to use scene_xxx functions and cooperate with anti-aliasing.</p></li>
  ...
  <li><p>The X3D feature to specify <b>height of the tallest object you can climb</b> inside <code>NavigationInfo.avatarSize[2]</code> field is now correctly supported. The demo model is inside <a href="https://castle-engine.sourceforge.io/demo_models.php">demo models</a> (open <code>demo_models/navigation/avatar_climb_stairs.x3dv</code>; only in SVN for now, until next release).</p></li>

  <li><p><b><code>VisibilitySensor</code> node</b> is implemented (looking only at frustum for now). See the testcase sensors_environmental/visibility_sensor.x3dv in <a href="https://castle-engine.sourceforge.io/demo_models.php">demo models</a> SVN.
  ...
  <li><p><a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> gets the <b>"Screenshot" button on the toolbar</b> (as it\'s an often used feature).
  ...
  <li><p><b><a href="https://castle-engine.sourceforge.io/blender.php">Our Blender X3D exporter</a> updated to be compatible to Blender 2.62</b> (and as close as possible to distributed original Blender exporter).
  ...
  <li><p><i>Developers:</i> a lot of <b>improvements to game data handling</b>, to make developing new games (like "Castle 2") more comfortable.</p>
    ...
    <ul>
      ...
      <li><a href="https://castle-engine.sourceforge.io/kanim_format.php">KAnim usage in "The Castle"</a> improved: <b>all castle animations are now separate kanim files</b>, and can be browsed e.g. by <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a>. "Castle 2" will not use kanim at all.</li>
  ...
  <li><p><i>Developers:</i> <b>picking improvements with T3D hierarchy</b>: unified <a href="https://castle-engine.sourceforge.io/castle.php">"The Castle"</a> style of picking, common to FPS games (pressing the "e" button when looking at item, stateless interaction with 3D items) with <a href="https://castle-engine.sourceforge.io/x3d_implementation_pointingdevicesensor.php">VRML/X3D pointer sensors</a> (operating with TouchSensors and drag sensors, that is stateful, tracks isOver/isActive and generally more powerful). Now it all goes through <code>T3D.PointingDeviceActivate/Move</code>, that can be overridden at each T3D.</p>
...
<a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a> and
<a href="https://castle-engine.sourceforge.io/engine.php">the engine</a>.
...
<ol>
  ...
  <li><p>New view3dscene menu item <i>"Help -&gt; Visit view3dscene website"</i>,
    ...
    and <a href="https://castle-engine.sourceforge.io/x3d_implementation_networking.php">Anchor
  ...
  <li><p>To our
    <a href="https://castle-engine.sourceforge.io/forum.php"><i>"Helping
  ...
  <li><p>We accept BitCoin for <a href="https://castle-engine.sourceforge.io/donate.php">donations</a>.
    If you wish to donate this way,
    simply send some bitcoins to this address: <code>1FuJkCsKpHLL3E5nCQ4Y99bFprYPytd9HN</code></p>

    <p>If you like view3dscene, please consider <a href="https://castle-engine.sourceforge.io/donate.php">donating
    using any of the listed options</a> :) Thanks!

  <li><p>For developers using our <a href="https://castle-engine.sourceforge.io/engine.php">engine</a>:
  ...
  <li><p>Various work on simplifying <a href="https://castle-engine.sourceforge.io/castle.php">"The Castle"</a> sources, and merging
...
Anchor {
  ...
  <li><p>Engine works fully under 64-bit Windows (Win64, Windows on x86_64).
    ...
    <a href="https://castle-engine.sourceforge.io/forum.php">forum</a>)
