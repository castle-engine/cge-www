...
<ol>
  ...
  <li><a href="https://castle-engine.sourceforge.io/x3d_implementation_text.php">X3D Text node</a> gets a new implementation, very fast, portable to OpenGLES (Android/iOS), and easy to use (shares tools and classes with 2D text rendering). You can also control <a href="https://castle-engine.sourceforge.io/x3d_implementation_text_extensions.php#section_ext_fontstyle_blending">whether it uses alpha blending or alpha test</a>.
  ...
  <li>Many improvements for 2D games. Besides Spine support, there\'s also a comfortable <a href="https://castle-engine.sourceforge.io/apidoc/html/Castle2DSceneManager.T2DSceneManager.html">T2DSceneManager</a> and <a href="https://castle-engine.sourceforge.io/apidoc/html/Castle2DSceneManager.T2DScene.html">T2DScene</a>. See <code>castle_game_engine/examples/2d_spine_game/</code> for example of use.
  <li>Many improvements for Android. See <a href="https://castle-engine.sourceforge.io/tutorial_mobile.php">new tutorial page about developing mobile games</a>. Try our "portable games skeleton" code in <code>castle_game_engine/examples/portable_game_skeleton</code>.
  <!-- This includes <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleWindow.TCastleApplication.html#OnInitialize">Application.OnInitialize</a> improvements (it now happens when GL context is ready, so you can really initialize your whole game there, with progress bar), fixes to <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleWindowTouch.TCastleWindowTouch.html#AutomaticTouchInterface">AutomaticTouchInterface</a> (it was not cooperating with our progress bar tricks previously). --> Also Android emulator is working now with our programs.
  <li>New X3D extensions (used by our Spine loader, but may also be useful for other purposes too):
    <ul>
      <li><a href="https://castle-engine.sourceforge.io/x3d_implementation_navigation_extensions.php#section_ext_blending_sort">NavigationInfo.blendingSort</a>
      <li><a href="https://castle-engine.sourceforge.io/x3d_implementation_interpolation_extensions.php#section_cubic_bezier_interpolator">CubicBezierXxx interpolators</a>
      <li><a href="https://castle-engine.sourceforge.io/x3d_implementation_texturing_extensions.php#section_texture_properties_gui_texture">TextureProperties.guiTexture </a>
      <li><a href="https://castle-engine.sourceforge.io/x3d_implementation_eventutilities_extensions.php#section_forceContinousValue_Changed">X3DSequencerNode.forceContinousValue_Changed</a>
    </ul>
  <li>High-quality image scaling, using interpolation modes like Mitchell, Blackman, Lanczos, Gaussian. See <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleImages.TCastleImage.html#MakeResized">TCastleImage.MakeResized</a> and <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleImages.html#TResizeNiceInterpolation">TResizeNiceInterpolation</a>. The implementation uses excellent fcl-image library underneath.
  <li>Named animations: Our engine detects named animations by looking at X3D TimeSensor node names. Name starting with "Animation_xxx" indicates an animation, with name "xxx". Some converters (right now, our Spine to X3D conversion) follow this convention, so our engine immediately "knows" the Spine animation names and can play them. The benefits:
    <ul>
      ...
      <li>TCastleScene offers a simple API for programmers to browse and run animation by name, see <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleSceneCore.TCastleSceneCore.html#PlayAnimation">TCastleScene.PlayAnimation</a>.
    </ul>
</ol>
...
<ul>
  ...
  <li><a href="https://castle-engine.sourceforge.io/mountains_of_fire.php">Mountains Of Fire &mdash; split-screen cooperative survival game</a>
...
<ol>
  ...
  <li><p><b>Comfortable API for 2D rendering</b></p>
    ...
    <ul>
      <li><p><a href="https://castle-engine.sourceforge.io/apidoc/html/CastleGLImages.TGLImage.html">TGLImage</a> is now the main class to draw images in 2D. It renders 2D images as non-power-of-two textures underneath, which makes it suitable for modern OpenGL and GLES, and better for anti-aliasing. The <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleGLImages.TGLImage.html#Draw">Draw</a> method can stretch the image, and the <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleGLImages.TGLImage.html#Draw3x3">Draw3x3</a> method can even intelligently stretch the image while preserving constant-size corners. It also automatically uses alpha channel (for alpha test or alpha blending, you can also force specific treatment using <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleGLImages.TGLImage.html#Alpha">TGLImage.Alpha</a> property). Property <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleGLImages.TGLImage.html#Color">TGLImage.Color</a> allows to easily color the rendered image. <a href="https://castle-engine.sourceforge.io/tutorial_player_2d_controls.php">Tutorial chapter about 2D controls contains various usage examples</a>.

      <li><p>All <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleControls.html">CastleControls</a> components use now <code>TGLImage</code> for all of their rendering. They use <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleControls.TCastleTheme.html">TCastleTheme</a> (as <code>Theme</code> global instance) to get their images, this way you can easily change the look of all controls defined in <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleControls.html">CastleControls</a>.

      <li><p>Dialog boxes in <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleMessages.html">CastleMessages</a> unit are also using <code>TGLImage</code> for the rendering. They also use normal buttons at the bottom, so they are clickable and generally look and behave like normal dialog windows with normal buttons :) Same thing for the progress bar.
      ...
      <li><p>Image and text drawing routines discourage now from using global state, like "current color" or "current raster position". You should use <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleGLImages.TGLImage.html#Draw">TGLImage.Draw</a> versions that take explicit draw position, and <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleFonts.TCastleFont.html#Print">TCastleFont.Print</a> versions that take explicit draw position and color.
    </ul>
  ...
  <li><p><b>Modern font loading and rendering</b></p>
     ...
     <p>Important font classes are called now <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleFonts.TTextureFont.html">TTextureFont</a> and (abstract) <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleFonts.TCastleFont.html">TCastleFont</a>. TTextureFont is either loaded from ttf or from prepared data (when TTextureFontData was converted to a Pascal code). There is also new <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleFonts.TSimpleTextureFont.html">TSimpleTextureFont</a> to draw a colorful text from glyphs in an image.
  ...
  <li><p><b>Multi-touch is implemented :)</b>
    ...
    <ol>
      ...
      <li>Multi-touch API changes:
        <ol>
          <li><b>Press/Release</b>: <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleKeysMouse.TInputPressRelease.html">TInputPressRelease</a> structure (which is a parameter for OnPress/OnRelease events) has a new field FingerIndex. On devices with a single normal mouse (no touch), like on desktops, FingerIndex is always 0.
          <li><b>Motions</b>: TUIControl has new method <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleUIControls.TInputListener.html#Motion">Motion</a>, and TCastleWindow/TCastleControl have new callback <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleWindow.TCastleWindowCustom.html#OnMotion">OnMotion</a>. They get a parameter <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleKeysMouse.TInputMotion.html">TInputMotion</a>, that contains new and old position, a FingerIndex, and Pressed describing which buttons are pressed (for mouse, this corresponds to actual buttons pressed, may be [] if none; for touch device, it is always [mbLeft]; this way you can just detect any dragging by Pressed <> []).
          <li><b>Current state</b>: TCastleWindow and TCastleControl have a list of <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleWindow.TCastleWindowCustom.html#Touches">Touches</a>, listing current state of the touches. It\'s length varies (can be zero if not touching). Right now, the state of touch is just it\'s position in 2D and FingerIndex, but we may extend it in the future. See comments at <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleUIControls.TTouch.html">CastleUIControls.TTouch</a> type.
...
<ol>
  ...
  <li><b>The default <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleCameras.TWalkCamera.html">TWalkCamera</a> inputs are now equal to <a href="https://castle-engine.sourceforge.io/apidoc/html/CastlePlayer.TPlayer.html">TPlayer</a> inputs, in particular they honour common AWSD combo</b>.
    <ul>
      <li>You can move using AWSD by default (e.g. in <a href="https://castle-engine.sourceforge.io/view3dscene.php">view3dscene</a>).
  ...
  <li>CastleEnumerateFiles API much changed and renamed to <a href="https://castle-engine.sourceforge.io/apidoc/html/CastleFindFiles.html">CastleFindFiles</a>. It supports searching for files inside Android assets too (although, unfortunately, not recursive &mdash; because of NDK API limitations).
