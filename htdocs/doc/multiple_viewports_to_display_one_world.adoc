...
cgeimg::float[
  ...
  view3dscene_viewports.png|Castle Model Viewer with 4 viewports
]
...
If you use link:sound[3D sound] in your application, for example through the cgeref:TCastleSoundSource[] behavior, you need to consider which viewport's camera should control the _"sound listener"_. The _sound listener_ determines the position and orientation of the "ears" in the 3D sound model, that determine which sounds (and how loud) go to left or right speaker.
...
* https://github.com/castle-engine/castle-engine/tree/master/examples/user_interface/zombie_fighter[zombie_fighter] example.

* https://github.com/castle-engine/castle-engine/tree/master/examples/viewport_and_scenes/multiple_viewports[multiple_viewports] example. Shows a window split into 4 viewports, all showing the same scene, but with a bit different effects.

* https://github.com/castle-engine/castle-engine/tree/master/examples/3d_games/explore_impressive_castle[explore_impressive_castle] example. Shows using additional viewport for an _"overview"_ camera looking at map from the top.
