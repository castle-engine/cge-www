...
+
The platform of all our packages is set by default to _"Windows 32-bit"_ and this is good for the Delphi 32-bit IDE. If you use the new Delphi 64-bit IDE, switch to _"Windows 64-bit"_ and install then.
+
NOTE: This platform selection is _independent_ from the platforms actually supported by the engine.  No matter how you install the packages, engine components will be available for all supported target platforms (including _Win32_, _Win64_, and link:delphi_linux[Linux64]; link:delphi_more_platforms[more platforms are coming]).
...
+
Restart Delphi IDE, as advised by the message that will appear.
+
