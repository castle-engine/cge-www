...
You have to install link:download#packages[the necessary libraries] for your system.
...
To have fully-working installation, build also link:castle-model-viewer[castle-model-viewer]. Editor executes it on double-click to view link:model_formats[all our scene formats, 3D and 2D: glTF, X3D, Spine JSON, sprite sheets etc.].
