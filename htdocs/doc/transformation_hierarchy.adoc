...
* At the beginning, don't merge the scenes. It's more natural, and in 90% cases perfectly fast, to use one cgeref:TCastleScene[] for each one model file you load. This allows to trivially load the model using `Scene.Load` and is really advised for most cases. link:optimization#section_combine_scene[See also the relevant section in the manual about optimization].

* 100, or even 1000, or cgeref:TCastleScene[] instances visible should not be a problem. You should consider merging them if you have 10 000 or more. It depends on your use-case (how complicated are the scenes, how heavy is their rendering and which optimizations matter most).
