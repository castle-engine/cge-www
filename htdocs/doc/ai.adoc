...
- Don't use AI in a way that makes you dumber. Don't make solutions that are unmaintainable, poorly understood, badly designed, mixed with hallucinated docs or assumptions.
...
- **Understand everything** you submit. Know the reasons behind it. Do your own research and decision-making.
...
- **AI-generated larger pieces of code / documentation are often wrong.** They confuse you (suggesting further work in the wrong direction), they show hallucinated or deprecated APIs, they do not use recommended approaches (e.g. AI often puts everything in one big program file, which makes the application non-link:cross_platform[cross-platform] and ignores our link:views[] concept to organize applications), they do not understand the platform-specific and compiler-specific language nuances etc.
...
[NOTE]
====
Unlike most of our engine documentation, parts of this text are written in my personal (https://michalis.xyz/[hi, I'm Michalis]) voice, sharing my personal experience. I hope this helps you understand the reasoning behind some of the statements.
...
- The same goes for documentation and issue/PR descriptions. Make sure it's good documentation, simple, and to the point. Documentation must actually help human developers, not be just _"something you had to provide, so you asked AI to produce something that looks plausible, to pass the review"_.
+
Especially watch out for overly long and marketing-style statements. We don't want long documents with emoticons claiming that _"this is the best solution in the world"_. And while being grammatically correct is nice, it's not the most important thing we look for in good documentation.
+
...
- Use AI to search for things (esp. things that are beyond "grep search"; e.g. you don't know how it is called, but you know that there exists _"something that allows you to add two 3D vectors"_; AI can find it and tell you to `var A, B, C: TVector3; A := B + C;`).
--
+
...
+
Don't blindly trust the review. If AI recommends this or that change, understand why, check whether the recommendations are suitable in this case, check that it's not hallucinating or misunderstanding how to change the code, in other words judge for yourself.
+
...
+
Don't throw a hard task at AI, hoping it will _solve it for you in one shot_. Split the task into smaller pieces (make a "TODO list" for the agent). Specify the conditions for the task (do X using Y and Z, e.g. "communicate with X using its REST API and engine TCastleDownload class", or "introduce a method to class X doing this"). Always consider how to refactor the solution to be simplest and most maintainable. AI output is supposed to be a starting point for you, not the final solution.
+
[NOTE]
====
Special recommendation applies to _starting a new project with AI_. *Do not* tell AI to "just create a new game using Castle Game Engine" -- as then it can make a bad starting point, ignoring our link:views[], going against our recommended setup for cross-platform projects, making visual designing impossible etc.

Instead, *tell AI to create a new project using `castle-engine create empty` command-line* (see our link:build_tool[build tool] commands) and then turn this into your desired game.
====
+
[NOTE]
====
Don't worry, _visual designing_ in our editor doesn't conflict with editing the project with AI agents. Our designs are just JSON files, and agents are capable of modifying these JSONs. So the agents can cooperate with visual designing effectively.

In fact, _do not_ let agents do all the work by writing Pascal code, when instead they should modify design JSON files. The AI agents tend to solve too many tasks by writing Pascal code, because the AI is happy to write long spaghetti boilerplate code. But this is not good for further modification -- the effect of setting up components using Pascal code can only be observed at run-time, you cannot edit the design in the editor. Guide the AI to add/modify components by editing our designs (`.castle-user-interface`, `.castle-transform` JSON files) when possible.
====
...
== Don't violate copyrights / licenses when using AI

=== Context: why this is a problem

//Sadly, in the process of training AI models, something very bad was done to the work of many open-source developers (including us).

The AI models have been trained on publicly available data, in many cases on open-source projects, ignoring copyrights and licenses of these projects. AI output doesn't attribute proper copyrights and so using it may violate licenses (in particular copyleft licenses like GPL).

Strictly speaking, the output of any tool should be covered by the same copyrights / licenses as the input it was processing, that was the rule that was obeyed before AI got popular. Yet AI companies have ignored this, the AI output is a mixture of various sources, without proper attribution or licensing for any of them. And indeed AI output can easily produce code that is identical or very similar to the code it was trained on. Sadly, copyright / licensing has been ignored by AI training and there is no one challenging this in court (successfully).

Watch the talk https://www.youtube.com/watch?v=xvuiSgXfqc4[AI generated violations of FOSS Licenses] for funny + sad examples.

[NOTE]
====
A special note addressing notions I've heard that _"copyright is outdated, we live in a post-copyright world etc."_. No: copyright is essential, in future as in the past.

Without the copyright and licenses, everything (including all open-source code, like our engine, and all proprietary code, like proprietary AI tools and training software) would be in the _public domain_. This means everything you may ever do (code, art) would not be automatically owned by you, thus you cannot e.g. gain from selling it (like selling proprietary software, or selling books!) or protect it (like making sure people are obligated to share improvements to your open-source code, like copyleft licenses do).

Copyright and licenses are still the basis of all work ownership (software and beyond).
====

=== Guideline: Do diligence to avoid copyright/license violations

Do **reasonable due diligence to make sure your AI-generated code doesn't infringe any copyrights or licenses**. This means that if you generate a significant portion of new code -> search for similar code, using normal web search, and using AI review, before committing it.

Again, see the https://www.youtube.com/watch?v=xvuiSgXfqc4[talk linked above] for examples how AI can easily produce code that is identical or similar to the code it was trained on, and omit (or even falsify) the original author and license. The problem is real.

Submitting code that is covered by a specific copyright/license, and not including that copyright/license, and ignoring the license requirements -> is a violation of the given license. We require that the code you submit be covered by _your_ copyright, and it's your responsibility to make sure that whatever AI generated can indeed be treated as such.

This is yet another reason to **generate _smaller_, not _larger_, pieces of code**. Code-completing a line of code with AI is probably not infringing any copyright/license, it probably mostly follows your own code a few lines above. On the other hand, generating a complete e.g. "_inverse square root implementation"_ is almost certainly a violation that will not be attributed to the actual, well-known author.

== See also

In our engine:

- We have experimented even with _vibe coding_ for fun, see https://github.com/michaliskambi/castle-vibes/ for the complete code and data, screenshots, and a https://github.com/michaliskambi/castle-vibes/blob/master/README.md[long README with observations]. We've made a https://castle-engine.io/wp/2026/02/23/ai-usage-guidelines-and-thoughts-updated-and-test-of-vibe-coding-with-castle-game-engine/[news post summarizing the conclusions].
...
- https://docs.kernel.org/process/coding-assistants.html[Linux kernel guidelines about AI] (in particular with explicit mention of copyright issues)

More opinions I agree with:

- https://www.youtube.com/watch?v=_zfN9wnPvU0[Kurzgesagt video warning about AI slop]. There is a real problem of welcoming hallucinations into our knowledge base (Internet).

- I do think that one answer is we need search engines trying to "filter out" AI slop (not amplify it). I'm a happy user of https://kagi.com/[Kagi] and https://blog.kagi.com/llms[agree with their POV about LLMs].

- https://substack.com/home/post/p-185933546[Article from Addy Osmani "The 80% Problem in Agentic Coding"] is a balanced viewpoint I agree with -- highlighting both the hope and the dangers of AI agents in coding.

- https://www.youtube.com/watch?v=AEmHcFH1UgQ[Interview with Pi devs] has lots of valid observations about AI and coding.
