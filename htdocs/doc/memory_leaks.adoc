...
For example you created an instance of a class by `MyInstance := TObject.Create` but then forgot to call `FreAndNil(MyInstance)`. See link:modern_pascal#_freeing_classes[freeing classes (modern Pascal introduction)]. While Pascal offers various helpers to make it easy to deal with (e.g. `TComponent` can own other `TComponent` instances, freeing them automatically) but in general Pascal remains a language where you have to manually free all your allocated memory.

While not all memory leaks result in severe consequences, they usually indicate unsafe code organization and may be result of more extensive bugs. Therefore it is highly recommended to keep the code clean of memory leaks.
...
Delphi has https://docwiki.embarcadero.com/Libraries/Sydney/en/System.ReportMemoryLeaksOnShutdown[ReportMemoryLeaksOnShutdown] global variable. Set it to `true` to detect and report memory leaks. It only affects Windows platform.
