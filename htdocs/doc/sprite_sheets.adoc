...
Once you have it, simply load it to cgeref:TCastleScene[], using the `TCastleScene.Load` method (or by setting `TCastleScene.URL`), play animation by `TCastleScene.PlayAnimation` (or setting `TCastleScene.AutoAnimation` and `TCastleScene.AutoAnimationLoop`) and generally use all the `TCastleScene` API. See CGE manual about link:viewport_and_scenes[viewports, displaying, transforming, animating scenes].
