...
* Platformer demo in link:https://github.com/castle-engine/castle-engine/tree/master/examples/2d_games/platformer[examples/2d_games/platformer/] is a complete platformer demo using this technique.
...
To add the sprite sheet to a cgeref:TCastleViewport[] (see link:viewport_and_scenes[viewports and scenes documentation]) create a cgeref:TCastleScene[] inside cgeref:TCastleViewport[] and assign scene `URL`, `AutoAnimation` etc. All standard methods to load and play animations work. You can do this by code (at run-time) or in the CGE editor (at design-time).
