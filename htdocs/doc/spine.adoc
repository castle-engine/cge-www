...
You can also add Spine animations using the https://castle-engine.io/manual_editor.php[editor], just add a cgeref:TCastleScene[] and set the scene `URL` to point to the JSON file.
...
When you run our https://castle-engine.io/manual_editor.php[editor], create a _"New Project"_ and use template _"2D Game"_. This shows a viewport with mutliple Spine scenes. A Spine scene is just a cgeref:TCastleScene[] instance and you add it to cgeref:TCastleViewport[] to make it visible, animated etc.
