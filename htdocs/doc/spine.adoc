...
Castle Game Engine https://castle-engine.io/ has advanced support for Spine http://esotericsoftware.com/ 2D animations.

Spine is a great software for designing 2D animations for games, using modern techniques (bones, skins, meshes etc.). Animations done this way are lightweight: their loading and rendering is very efficient, even on mobile devices like Android or iOS. But they are also very flexible:
...
See our https://castle-engine.io/documentation.php[engine documentation] for an overview of how to use the engine.

When you run our https://castle-engine.io/manual_editor.php[editor], create a _"New Project"_ and use template _"2D Game"_. This shows a viewport with multiple Spine scenes. A Spine scene is just a `TCastleScene` instance and you add it to `TCastleViewport` to make it visible, animated etc.

To play a Spine animation,

* In editor or from code set cgeref:TCastleSceneCore.AutoAnimation[TCastleScene.AutoAnimation].

* Or from code call cgeref:TCastleSceneCore.PlayAnimation[TCastleScene.PlayAnimation] method. Use cgeref:TCastleSceneCore.Animations[TCastleScene.Animations] to list available animations.

See link:viewport_and_scenes[viewport and scenes] for more information about using viewports and scenes -- there's nothing special about scenes using Spine model.
...
////
Dragon Bones seems not maintained anymore, with broken websites and even downloads.
...
You can also find some 2D spine animation assets in thematic game art & development websites. Here's an example of free spine 2D animation for pirate characters https://retrostylegames.com/portfolio/pirate-characters-spine-2d-animation-unity-assets-free-download/
