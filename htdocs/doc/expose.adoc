...
+
It is more powerful and more obvious than alternative cgeref:TCastleScene.CastGlobalLights[] (which also allows scene lights to shine on other objects, but in that case lights stay hidden inside the scene, they are only accessible as X3D nodes).
