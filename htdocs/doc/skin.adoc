...
In most cases, you don't need to do anything special on the engine side to use it. Simply design models using link:blender[Blender] with Blender's _armature_ and _bones_, export it to link:gltf[glTF], and load it in _Castle Game Engine_ as usual.
...
We try to perform the animation on the GPU, which means it's very fast. To demonstrate this, our https://github.com/castle-engine/castle-engine/tree/skinned-animation-gpu/examples/animations/animate_bones_by_code[examples/animations/animate_bones_by_code] demo has a button _"Add Crowd"_. We also have a fallback (for ancient GPUs) implementation that animates on the CPU.
...
Build and run example https://github.com/castle-engine/castle-engine/tree/skinned-animation-gpu/examples/animations/animate_bones_by_code[examples/animations/animate_bones_by_code] to see how to do this.
...
NOTE: If you're familiar with the X3D specification syntax for node definitions, you can find https://github.com/castle-engine/castle-engine/blob/a54d85208b42015bc3875870d62b3f49106291b4/tools/internal/x3d-nodes-to-pascal/nodes-specification/CastleEngineExtensions.txt#L257[`Skin` node spec expressed in this way here].
