...
- You can just write X3D content in a text file, with extension `.x3dv` ("classic" syntax) or `.x3d` (XML syntax). The node definition then looks like `Skin { ... }`. See TODO[section below] for an example.

- You can build the X3D nodes graph in Pascal code. In this case, you create an instance of the cgeref:TSkinNode[] class. The properties of this class map 1-1 to the `Skin` node fields.

== Overview

// TODO: Link to Pascal docs for each property
Skin {
  ... // TODO: copy from CastleEngineExtensions.txt
}
...
Skin {
    ...
    # in the "shapes" list.
    #
    # You will typically use X3D USE mechanism to refer to existing
    # joint nodes here. It is technically possible (because it has to be in X3D)
    # to just list here full joint nodes' contents with DEF, and then use USE
    # for the skeleton, but this can results in a hard-to-read (for humans;
    # computers don't care) content.
    #
...
- Transforming glTF skinned animation information into X3D SkinnedAnimation is straightforward.

- Implementing SkinnedAnimation, including implementing it on GPU (skin is applied in shaders) is as straightforward as in glTF. You can follow the glTF cheat sheet on https://www.khronos.org/files/gltf20-reference-guide.pdf that describes the GPU implementation on skinning in 1.5 pages.
...
- Our SkinnedAnimation, just like glTF approach, has an obvious, simple and efficient GPU implementation.
...
- Note that there are also some similarities. E.g. `HAnimHumanoid` and `Skin` have `joints` and `skeleton` fields with mostly the same purpose and meaning.

For people that want to use H-Anim nodes in CGE, nothing changes, we continue to support them. Moreover, we can consider in the future transforming H-Anim nodes under the hood to SkinnedAnimation, to make them as efficient to render as SkinnedAnimation, using GPU.
