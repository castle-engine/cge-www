...
*PLY (Polygon File Format)* is a simple format used in 3D scanning and 3D printing.

We support reading models in this format.

- ASCII and binary (with any endianness) versions supported.
- Point clouds (no faces) or regular meshes (with faces) supported.

For overview of this format see:

- https://people.sc.fsu.edu/~jburkardt/data/ply/ply.html[PLY Files - by John Burkardt]
- https://paulbourke.net/dataformats/ply/[PLY Polygon File Format - by Paul Bourke]
- https://en.wikipedia.org/wiki/PLY_(file_format)[Wikipedia: PLY file format]

Testcases:

- https://sketchfab.com/3d-models/kaktus-ply-7b7cc7188f17468595506500e186a9c0[kaktus PLY model from Sketchfab (can be downloaded as PLY source)]
- https://github.com/castle-engine/demo-models/tree/master/ply[Our demo-models, ply subdirectory]
- Above overviews also link to various test PLY files.
