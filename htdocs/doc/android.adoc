...
You can create games (and other applications) for Android using _Castle Game Engine_. This page describes installation of all the necessary software.

See also link:android_faq[Android FAQ], link:android_sdk_and_ndk_troubleshooting[Android SDK and NDK troubleshooting].

Examples: Almost all our examples, and all new projects created by CGE editor _"New Project"_ from one of the templates, are automatically https://castle-engine.io/manual_cross_platform.php[cross-platform] and can be compiled for Android. In particular:

* create _"New Project"_ from any template - 2D, 3D etc.
* `examples/mobile/simple_3d_demo/` - shows 3D world and navigation and graphic effects on Android,
* `examples/mobile/achievements/` - shows 2D animation and Google Play Games integration on Android,
...
Use the link:build_tool[Build Tool] to easily compile these programs for Android (or other platforms -- they all can be compiled as standalone games too!). But first be sure to install the Android tools described below. They will be called by the link:build_tool[Build Tool] under the hood.
...
. Then compile a standalone version using our link:build_tool[Build Tool], by calling
+
----
 castle-engine compile
----

. Now compile the Android application using our link:build_tool[Build Tool], by caling
+
----
