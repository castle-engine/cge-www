...
[NOTE]
====
Alternatively, you can skip this step (Java installation) and in the next step install full https://developer.android.com/studio[Android Studio]. The complete _Android Studio_ will already include Java version suitable for Android SDK.

For your information, the Java installation can be located by going to _"File -> Settings"_ and there to _"Build, Execution, Deployment -> Build Tools -> Gradle -> Gradle JDK"_. See the screenshot below.

cgeimg::block[android_studio_java_location.png|Java location in Android Studio]

If necessary, you can point to this Java location using Castle Game Engine link:editor[] (open _"Preferences"_ dialog, go to the _"Android"_ page, set _"Java Location"_) or by setting the `JAVA_HOME` environment variable. Though in many cases it is not necessary to do this, Android tools will find Java automatically.
====

=== Install Android SDK (maybe along with Android Studio)

Download and install _Android SDK_.

There are two ways to do it:

1. Install the whole https://developer.android.com/studio[Android Studio].
+
This is the easier way (some configuration can be then done using GUI) and comes with useful additional GUI tools to manage virtual Android devices or mirror real (physical) Android devices. We advise this option.
+
For your information (you will need this later), location of the Android SDK is visible in the Android Studio _File -> Settings -> Languages & Frameworks -> Android SDK_. You need to have some project open to access this _Android Studio_ dialog -- just create a new empty project if you don't have any.
+
...
2. Alternatively: Install _Command line tools only_ (scroll to the bottom of https://developer.android.com/studio ):
+
--
...
=== Let Castle Game Engine know your Android SDK location

The _Android SDK location_ is the directory that contains subdirectories like `cmdline-tools`, `tools`, `platform-tools`. See above how to find where your Android SDK is installed.
...
2. Alternative: If you want to use our command-line link:build_tool[build tool] or some Android command-line tools, you can set instead the `ANDROID_HOME` environment variable.
+
