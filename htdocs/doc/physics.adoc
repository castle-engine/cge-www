...
To make any cgeref:TCastleTransform[] affected by physics add there a collider (any cgeref:TCastleCollider[] descendant) and a rigid body (cgeref:TCastleRigidBody[]). In the editor, adding any collider automatically also adds the cgeref:TCastleRigidBody[], so *you only need to do one step: add a collider*. The usual way to do this is by right-clicking on a cgeref:TCastleTransform[] instance in your hierarchy, and choosing from context menu _"Add Behavior -> Physics -> Collider -> ..."_.
