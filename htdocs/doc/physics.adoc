...
--
+
NOTE: The value doesn't matter at design-time (in CGE editor). To have reliable object picking in editor, at design-time we internally always create collisions strutures, ignoring cgeref:TCastleSceneCore.PreciseCollisions[] value. Otherwise picking smaller scenes (like creatures inside a level) would be impossible in some cases, because the big level scene would collide as a big box that includes everything.
