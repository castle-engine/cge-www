...
WARNING: The _"new physics"_ features described here are not yet available in CGE "master" branch, nor in the link:download[downloads] we recommend. They are on https://github.com/castle-engine/castle-engine/tree/physics[physics branch] and more (joints) in https://github.com/castle-engine/castle-engine/tree/physics_j[physics_j branch]. But they will be *soon*! We work on making them officially available in October 2022.
...
////
TODO: Make links below using cgeref, once possible (on master).
////

- `TCastleRigidBody`
- Family of components descending from abstract `TCastleCollider`:
+
--
- `TCastleSphereCollider`
- `TCastleBoxCollider`
- `TCastleCapsuleCollider`
- `TCastleMeshCollider` (only static bodies)
- `TCastlePlaneCollider` (only static bodies)
--

Just add to any cgeref:TCastleTransform[] (usually to cgeref:TCastleScene[] or a primitive like cgeref:TCastleBox[], cgeref:TCastleSphere[]) one `TCastleRigidBody`, one collider, and observe that it's affected by
...
You can visualize physics colliders (esp. useful if you disable the `AutoSize` on them and want to see the effect) using the menu item _"Physics -> Show Colliders"_.
...
- add `TCastleRigidBody` and some collider to the object you want to act as a bullet
- set `TCastleRigidBody.LinearVelocity` to something non-zero.
