...
- Potentially increase the gravity strength by editing cgeref:TPhysicsProperties.GravityStrength[Viewport.Items.PhysicsProperties.GravityStrength].
+
The default `GravityStrength` is `9.81`, which makes sense for 3D games when 1 unit is 1 meter.
+
But 2D games often use different measurements. By default, the link:viewport_2d[viewport we advise to create for 2D games] spans 1000 units in height. So a larger gravity strength is necessary to make the objects fall down in a reasonable time.
...
See link:viewport_2d[overview of how 2D works in our engine].
...
[#query_collisions]
== Querying For Collision (ray-casting, sphere-casting)

Use the cgeref:TCastleAbstractRootTransform.PhysicsRayCast[] to _"cast a ray"_ and see whether it hits something. This means we calculate the first 3D object that would collide with a 3D ray, which starts in a given 3D point and goes in the given 3D direction.

- The cgeref:TCastleAbstractRootTransform.PhysicsRayCast[] takes and returns values (ray origin, direction, returned distance, point) in the _world coordinate system_.
...
You can also use _sphere casting_ using the cgeref:TCastleAbstractRootTransform.PhysicsSphereCast[] or cgeref:TCastleRigidBody.PhysicsSphereCast[].

- These methods are similar to casting a ray, but now we move a _sphere_ (with given radius, from given position, along given direction) and see what will be hit by this sphere.

- You can think that _casting a ray_ is like a special case of _casting a sphere_, but a ray is like an infinitely small sphere (with radius = zero).
...
More involved example is in https://github.com/castle-engine/castle-engine/tree/master/examples/2d_games/platformer[examples/2d_games/platformer].
