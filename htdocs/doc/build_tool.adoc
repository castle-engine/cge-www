...
In most cases, you will use link:editor[Castle Game Engine Editor] (`bin/castle-editor` executable) that automatically uses build tool described here, under the hood. Alternatively, you can execute the build tool directly, on the command-line, using `bin/castle-engine` executable.
...
You can customize how we call Delphi:

- `--delphi-version` instructs to use given Delphi version, like `23.0` for Delphi 12, `37.0` for Delphi 13. The numbers we expect correspond to the https://docwiki.embarcadero.com/RADStudio/Florence/en/Compiler_Versions["Product Version" of Delphi] and the names of subdirectories in `C:\Program Files (x86)\Embarcadero\Studio`.
+
For example, to compile with Delphi 13, call:
+
```
castle-engine compile --delphi-version=37.0 --compiler=delphi
```

- `--delphi-use-dcc` instructs to use `dcc` command-line compiler instead of `msbuild`. See the https://github.com/castle-engine/castle-engine/blob/master/tools/build-tool/delphi_dcc_vs_msbuild.md[dcc or msbuild] discussion for details.
...
Output a given project information, like a project name if you call `castle-engine output name`.

This information is determined, more or less directly, from the link:project_manifest[CastleEngineManifest.xml] contents. For some scripts / CI jobs it is beneficial to have access to this information.

The next parameter, after `output`, determines what information to output. Available values are:
...
- `package-name`
+
Outputs the package name (like a .zip file) that _would_ be created by running `castle-engine package`.
+
This option (and only this) honors all the options of `castle-engine package` that influence the resulting package, like `--target`, `--os`, `--cpu`, `--package-format` and `--package-name-no-version`.
...
Here's a sample session using this:

```shell
$ cd /path/to/castle-model-viewer

$ castle-engine output name
castle-model-viewer

$ castle-engine output version
5.3.0

# This was tested on Linux/x86_64, so the default package name reflects this.
$ castle-engine output package-name
castle-model-viewer-5.3.0-linux-x86_64.tar.gz

# We can also ask what package name would be created for other platforms.
$ castle-engine output package-name --os=win64 --cpu=x86_64
castle-model-viewer-5.3.0-win64-x86_64.zip

$ castle-engine output package-name --os=darwin --cpu=aarch64
castle-model-viewer-5.3.0-darwin-aarch64.zip
```

=== output-environment

Output some environment information (independent of any project).

Next parameter determines the information:
