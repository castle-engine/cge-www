 ...
 ** For the Android (when `--target=android` or `--os=android --cpu=aarch64` or `--os=android --cpu=arm`), we create an `.apk` file with your application, ready to be installed on your device and distributed to testers.
...
Right now this is useful only on Android. So use the `install` command together with `--target=android` or `--os=android --cpu=aarch64` or `--os=android --cpu=arm`, like:
...
* Use `--os`, `--cpu` or `--target` to specify target operating system/processor (by default, we install for the current standalone platform, which will likely fail -- as this supports only Android).
