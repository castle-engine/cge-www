...
If you have a large number of small shapes using the same shader, consider turning on cgeref:TCastleViewport.DynamicBatching[]. This will internally detect and merge multiple shapes into one just before passing them to the GPU. In some cases, it is a very powerful optimization, reducing the number of _draw calls_.

Watch the cgeref:TRenderStatistics[MyViewport.Statistics.ToString] to see whether it reduces the number of rendered shapes.
