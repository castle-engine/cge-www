...
- Perform *link:alpha_bleeding[alpha bleeding]* (fixes the artifacts when scaling textures with transparent parts).
...
All link:cli_options_window[standard options understood by our GUI programs]
and all link:cli_options_common[standard options understood by all our programs]
are handled as well.
