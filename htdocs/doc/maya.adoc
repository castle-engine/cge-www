...
== Export to X3D with RawKee

https://github.com/und-dream-lab/rawkee[RawKee] project is an actively developed full-featured X3D exporter for _Maya_. It exports many features including meshes, PBR materials and skinned animation (using either H-Anim or our link:skin[`Skin` node]).
...
To export animations, you can export each keyframe as a still OBJ file, and then create a link:castle_animation_frames[simple XML file with the .castle-anim-frames extension to define an animation].
