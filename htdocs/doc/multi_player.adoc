# Multi-player
include::common.adoc[]
:description: Networking and multi-player solutions that can be used with Castle Game Engine. TCastleDownload for HTTP(S) and REST communication. CastleClientServer using Indy for classic client/server over TCP/IP. mORMot 2 (for ORM, REST communication, WebSockets and more). RNL for real-time multi-player. Plans for Nakama integration.
...
[NOTE]
====
Applications should use cgeref:CastleHttps[] unit to enable HTTPS support. Just add it to the uses clause of one of your units (like `GameInitialize`). With some platforms and compilers, this unit actually does nothing (but it's harmless to still use it), while on others it is necessary to enable HTTPS support (it enables OpenSSL usage).
====
...
- https://github.com/castle-engine/castle-engine/tree/master/examples/network/random_image_from_unsplash[examples/network/random_image_from_unsplash/] (download random image from Unsplash),
...
- https://github.com/castle-engine/castle-engine/tree/master/examples/network/ask_openai_assistant[ask_openai_assistant (in examples/network) shows talking with an OpenAI assistant (essentially your own, customized ChatGPT)] (sends a series of HTTP POST and GET messages to talk with OpenAI server).
