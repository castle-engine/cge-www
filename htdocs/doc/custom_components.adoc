...
In general there are 2 ways to make your component be a composition of other components:

=== Adding internal children components
...
initialization
  ...
  R.OnCreate := {$ifdef FPC}@{$endif}TMyRectanglesGroup{$ifdef FPC}(nil){$endif}.CreateInitialChildren;
