...
=== Simple property types and their default value

Following Pascal rules, properties can have a `default` specifier. This should specify the property value that is always initialized by the constructor. When serializing (writing to file) given component, we know we *do not* need to write the property value when it is equal to its default value.

NOTE: Remember that you still have to initialize in constructor the property to a given value. Merely adding `default` does not actually initialize the property. It it only an information for serialization, that it can safely assume that this property is initialized.
...
The above class always defines constants `DefaultRows` and `DefaultColumns`. While it is not required to define such constants, we advise doing it for all properties that have a non-obvious default value. This allows to easily set the default value from the code.

An alternative way to specify when should the property be stored is to use a `stored` method.

A property without the `default` specifier or `stored` method is always serialized to file. _Except for some specific property types, like floats and Strings. See below_.
...
- `Single` type can have a default value in FPC, but not Delphi. As our CGE editor right now uses only FPC/Lazarus, and the editor is where proper `default` really matters (because it serializes the designs), this is acceptable for us. Just put use `{$ifdef FPC} around it, like:
...
If you don't specify a default value for `Single` property, then it behaves as if it has default value `0.0`. Use the `nodefault` specified if you really want to save the `Single` property always.

Your final solution, when the above is not enough, is to use a `stored` method. This is useful if
...
- You want to avoid storing when the field value is different than default by some epsilon.
...
- The children component should have some non-empty name (to be nice for user) and this name should be unique in the owner. Do this using cgeref:InternalProposeName[], like `FMyChild.Name := InternalProposeName(TMyChildComponent, Owner);` .
...
begin
  ...
  Child.Name := InternalProposeName(TCastleRectangleControl, Owner);
  Child.Color := Red;
