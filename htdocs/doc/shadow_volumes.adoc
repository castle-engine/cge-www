...
cgeimg::block[
  ...
  manifold_colors_2.png|"Silhouette and Border Edges" fill mode
]
+
...
+
NOTE: Some edges that should be _white_ can be shown as _blue_, if only you have _some_ edges that are _blue_. That's because our auto-detection stops as soon as it is evident that scene is not 2-manifold. So if you see some _blue_ edges, fix them to be 2-manifold, just be aware that _some_ of them don't require fixing if they already should have a match in different shape.
--
