...
+
Models in In X3D can also define a default camera, using X3D `Viewpoint` or `OrthoViewpoint` nodes. If you write X3D files by hand, you can even generate such nodes using the _"Clipboard -> Print Current Camera (Viewpoint)"_ menu item in link:view3dscene.php[view3dscene].
