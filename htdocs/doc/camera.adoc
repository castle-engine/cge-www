...
- Another approach is to just set the camera vectors by code during your state start. Call cgeref:TCastleTransform.SetView[Viewport.Camera.SetView] like this:
+
[source,pascal]
----
...
** For example you can attach a weapon carried by a player in FPS game as a child of camera.
