...
Mouse look requires 2 features which are not yet implemented with the _Cocoa_ backend:

- `TCastleWindow.SystemSetMousePosition`
- Cursor hiding in `TCastleWindow.UpdateCursor`

You can take a look at the comments in https://github.com/castle-engine/castle-engine/blob/master/src/window/castlewindow_cocoa.inc[castlewindow_cocoa.inc] for more details. We did try to make them work -- alas, with no success yet. Any help is welcome!
