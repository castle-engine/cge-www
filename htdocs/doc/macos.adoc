...
Due to Apple _"translocation"_ mechanism (that internally executes the application from a random place on disk, if it was part of a zip downloaded from the Internet) you will likely have to set _Castle Game Engine_ path in _"Preferences" -> "General"_. There will be a warning, guiding you to do it, on the initial editor form.
...
2. In the future, CGE will likely have an alternative https://www.vulkan.org/[Vulkan] renderer. A new API, harder to use, more low-level, but with possibilities to deliver amazing performance and also scale better to multiple cores.
+
