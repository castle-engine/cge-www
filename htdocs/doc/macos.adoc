...
- Open terminal (_/Applications/Utilities/Terminal_) and execute `xcode-select --install`.
...
Just double-click the `castle-editor` application to run our editor... but first please follow the following section to deal with unsigned applications:
...
Make sure in _"Preferences -> FPC and Lazarus"_ that FPC version has also been detected correctly. This also implies that FPC binaries are properly trusted, as described above. You can ignore missing _Lazarus_, using _Lazarus_ for CGE development is optional -- you can use any text editor like link:vscode[VS Code].
