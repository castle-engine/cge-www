...
#### Allow running unsigned applications on newer macOS versions (remove quarantine bit)

When trying to run an unsigned application downloaded from the Internet, like our engine link:editor[editor] or link:castle-model-viewer[Castle Model Viewer], latest macOS versions give very misleading error message: _""xxx.app" is damaged and can't be opened. You should move it to the Bin"_. Nothing is actually damaged, just the application is placed in a _"quarantine"_. You can confirm it by executing in _Terminal_ commands like this:

```shell
# for Castle Model Viewer
xattr -l ~/Downloads/castle-model-viewer.app
# for Castle Game Engine editor
xattr -l ~/Downloads/castle_game_engine/bin/castle-editor.app
```

You will see a line with `com.apple.quarantine` in the output.

To fix it, use `xattr -cr ...`, like this:
...
#### Allow running unsigned applications on older macOS versions (avoid "developer cannot be verified" error)

Our editor (`castle-editor`), other tools (`castle-engine`, `castle-model-viewer`, `castle-image-viewer`), and the bundled FPC compiler are _unsigned_ applications for macOS right now. Running them for the first time will result in an error along the lines _"developer cannot be verified"_.

Solution: Before using the engine, you have to right-click on each of the binaries from _"Finder"_, choose _"Open"_ from the context menu, and then you will be able to confirm that you want to open an unsigned application.

So before using the engine please run all the binaries in `bin` subdirectory once, by right-clicking on them, and accept the warning that they are unsigned applications. Do this at least for:

--
- `bin/castle-editor`
- `bin/castle-engine`
- `bin/castle-image-viewer`
- `tools/contrib/fpc/bin/fpc`
- `tools/contrib/fpc/bin/fpc-cge`
- `tools/contrib/fpc/bin/ppcx64` (on Intel macs)
- `tools/contrib/fpc/bin/ppca64` (on Apple Silicon macs)
--

Note that some of the above (`castle-engine`, FPC compiler binaries) are command-line programs, and running them like this will just print the help message. Tat's fine, you can close the terminal window they open. You just run them to make macOS remember that you trust this application.
...
+
Due to Apple _"translocation"_ mechanism
...
you may have to set engine path manually there.
Whether this is actually necessary depends on macOS version and whether you removed the quarantine bit (see above).
...
+
NOTE: You can ignore missing _Lazarus_, using _Lazarus_ for CGE development is optional -- you can use any text editor like link:vscode[VS Code].
...
### Mouse look is untested

Mouse look requires 2 features which are... implemented, but not completely tested at this moment, as they cannot be completely tested over remote connection (like _Team Viewer_ or _VNC_, and this is how we have access to our macOS machine):

- `TCastleWindow.SystemSetMousePosition`
- Cursor hiding in `TCastleWindow.UpdateCursor`

You can take a look at the comments in https://github.com/castle-engine/castle-engine/blob/master/src/window/castlewindow_cocoa.inc[castlewindow_cocoa.inc] for more details. Any help with testing is welcome!
...
We hide these warnings in most cases (our link:build_tool[build tool] filters them out). But sometimes you may still see them, e.g. during the link:compiling_from_source.php[compilation of the build tool itself (if you do this)] or if you compile using link:lazarus[Lazarus IDE].
