...
This page documents _Castle Game Engine_ 7.0 new features. It's a work in progress -- as we work on making *final official CGE 7.0* released.

== Plans and Timeline

- We will make another _alpha_ release in 2024, called _7.0-alpha3_. The new features are documented below.
...
- The underlying https://github.com/castle-engine/pascal-language-server[LSP server] usefu also with Emacs, NeoVim and other text editors supporting LSP.
--
* Delphi:
+
--
...
* `castle-data:/` protocol (more advised than previous `ApplicationData` function). The "`data`" directory is documented link:data[in a new manual page]. It is also listed on a reworked manual page about networking, downloading and URLs https://castle-engine.io/manual_network.php.
