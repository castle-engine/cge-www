...
* The exact Delphi version support from _Castle Game Engine_ depends on the target platform, since the support for non-Windows platforms isn't always perfect in older Delphis:
+
--
Windows (32-bit and 64-bit):: Delphi >= 10.2.1
Linux (64-bit only):: Delphi >= 11.3 (see https://github.com/castle-engine/castle-engine/issues/577[#577 for reasons])
Android, iOS, macOS (Upcoming):: Delphi >= 10.4 (see below for reasons; it may change once we finalize these platforms)
--
