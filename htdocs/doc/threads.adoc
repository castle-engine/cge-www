...
+
Maybe we can make (but even this is far future) some clear separation, e.g. being able to use classes like cgeref:TCastleScene[], `TDrawableImage` from separate threads, but only if you use different instances of them.
+
