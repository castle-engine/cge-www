...
+
So to e.g. load textures _to GPU_ while doing a smooth rendering from another thread, we would need to implement another renderer some day. E.g. Vulkan allows to use multi-threading. Although the burden of protecting multi-thread access in Vulkan lies in the hands of user, so it's not something trivial to take advantage of anyway, but at least it becomes possible. See:
+
--
