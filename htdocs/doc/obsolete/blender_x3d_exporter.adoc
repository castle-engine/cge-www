...
* It *does not support animations*. To export animations from Blender that can work in https://castle-engine.io/[Castle Game Engine] and https://castle-engine.io/view3dscene.php[view3dscene], use our link:blender[Blender][Blender castle-anim-frames exporter] instead. The Castle-anim-frames exporter internally exports to a series of X3D files, so the rest of this page still matters to you anyway:)
