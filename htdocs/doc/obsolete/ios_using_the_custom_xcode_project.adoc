...
**Deprecated: The approach described on this page is deprected. The latest CGE has a better approach to create cross-platform games, including iOS. It is described on link:cross_platform[Cross-platform apps] page. Generally it just works out-of-the-box with all CGE projects. It uses the link:build_tool[build tool] to compile and package your game for iOS. You can even link:ios_services[integrate with a number of services like Apple Game Center], and the build tool will take care of everything (generating the necessary XCode project to actually release your app for iOS).**
