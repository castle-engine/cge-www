...
* You can set `Scene.Attributes.BlendingSort` to `bs2D` for 2D models. This makes their sorting faster. When importing link:spine[Spine] models, they automatically have `NavigationInfo.blendingSort = "2D"` so they automatically use this sorting.
