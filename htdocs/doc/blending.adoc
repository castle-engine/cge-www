...
You need to do the sorting explicitly.

- For 2D:
+
From the CGE editor, do the sorting using _"Viewport -> Sort Items For Correct 2D Blending"_ menu item.
+
From code, you can do this sorting by calling the cgeref:TCastleTransform.SortBackToFront2D[]  method like `Viewport.Items.SortBackToFront2D`. You should call it always after:
+
--
...
- For 3D:
+
From the CGE editor, do the sorting using _"Sort Items For Correct 3D Blending (from current view)"_ menu item.
+
From code, you can do this sorting by calling the cgeref:TCastleTransform.SortBackToFront[]  method like `Viewport.Items.SortBackToFront(bs3D, Viewport.Camera.WorldTranslation)`. You should call it always after:
+
--
- adding a partially-transparent object,
- or moving partially-transparent object,
- or moving the camera.
--

TODO: The need to manually do the sorting is definitely something we want to remove in the future engine releases. The engine should internally render the shapes, from all scenes, in the proper order for blending.
