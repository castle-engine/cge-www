...
* Within a viewport (`TCastleViewport`) instance, you place your 3D or 2D world (in `Viewport.Items`).
* A viewport always renders using a chosen camera in `Viewport.Camera`. This camera determines projection settings and position, rotation of the viewer. This camera determines which part of the world (your `TCastleTransform` instances) is visible.
* A hierarchy of `TCastleTransform` is placed inside `TCastleViewport.Items`. These are 3D or 2D objects, rendered from the point of chosen camera.
