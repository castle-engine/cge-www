...
For game objects (not user-interface), the closest equivalent of `GameObject` in _Castle Game Engine_ is the cgeref:TCastleTransform[] class. Instances of cgeref:TCastleTransform[] can be arranged in a hierarchy, where each element transforms (moves, rotates, scales) the children. The most important class of CGE is the cgeref:TCastleScene[], which is a descendant of cgeref:TCastleTransform[] that can actually render/animate/collide a model. Simply set cgeref:TCastleSceneCore.Url[TCastleScene.Url] to load your model (like a glTF file) there.
...
In the simplest case, for each 3D model, you create a cgeref:TCastleScene[] instance. You can do this visually, using the https://castle-engine.io/manual_editor.php[CGE editor], or from code.
...
Test your models with https://castle-engine.io/castle-model-viewer[Castle Model Viewer] to see what is supported. If you double-click on a model from https://castle-engine.io/editor[editor], it will automatically launch _Castle Model Viewer_.
...
Use cgeref:TCastleDownload[]. Similarly to `UnityWebRequest`, this class can send HTTP requests (GET, POST and others) and can download data asynchronously. It has been designed as an easy but also powerful way to communicate over the network. It also handles getting data from non-network requests (`file://` and other protocols) so you can easily switch your URLs.
