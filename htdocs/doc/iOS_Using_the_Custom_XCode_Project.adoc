...
**Deprecated: The approach described on this page is deprected. The latest CGE has a better approach to create cross-platform games, including iOS. It is described on link:manual_cross_platform.php[Cross-platform apps] page. Generally it just works out-of-the-box with all CGE projects. It uses the link:pass:[Build Tool][] to compile and package your game for iOS. You can even link:pass:[iOS Services][integrate with a number of services like Apple Game Center], and the build tool will take care of everything (generating the necessary XCode project to actually release your app for iOS).**
