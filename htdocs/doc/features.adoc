...
* Tiled maps (TMX files) can be loaded into cgeref:TCastleTiledMap[] for a 2D or 3D display in a viewport.
* Viewport display means they can be combined with other scenes on top of it, like sprite sheets for units on the map.
* See https://castle-engine.io/tiled_maps[manual about Tiled map].
