...
- By default, on platforms other than web, we use _OpenAL_ to play sounds. It's a great full-featured open-source audio library, perfect match for our open-source game engine.
...
- On the link:web[], we use _WebAudio_ backend, utilizing audio support in all modern web browsers.

[#formats]
== Sound file formats

We guarantee support for these formats on all platforms:

- _WAV (uncompressed, PCM)_ and
- _OggVorbis_

The engine ships with necessary libraries (like DLLs for Windows) to make these formats really work everywhere out-of-the-box.

Some backends support additional audio formats. This applies now to the link:fmod[FMOD] and link:web[] audio backends, that support additional formats like MP3 and FLAC. Refer to their documentation for details.
