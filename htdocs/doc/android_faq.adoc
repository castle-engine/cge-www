...
For more possibilities, you can write data to URLs returned by cgeref:ApplicationConfig[] function. You can write to it using all engine routines, like cgeref:StringToFile[], cgeref:UrlWriteXML[], cgeref:TTextWriter[], cgeref:UrlSaveStream[]. See link:url[Saving resources to URLs].
