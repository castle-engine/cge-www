...
This is a mobile version of link:castle-model-viewer[Castle Model Viewer], sharing the capabilities with the desktop version (all supported model formats, navigation, viewpoints, animation, screenshots) but with UI streamlined for mobile devices. It is available now for Android, soon also for iOS.
