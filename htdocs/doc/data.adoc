...
NOTE: The files inside this directory should only be read (not modified) at run-time to make you sure that your application works in all contexts (e.g. with Android read-only assets or system-wide installations on desktops). If you want to write files, use the cgeref:ApplicationConfig[] to get URL for writing configuration files.
