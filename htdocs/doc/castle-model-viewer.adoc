...
<ul>
  ...
  <li><p><i>Animations</i> may be played from VRML / X3D files,
    ...
    <p>You can activate X3D pointing-device sensors by clicking with
...
<dl class="params_list">
  ...
  <dd><p>Use full-screen anti-aliasing. You can also configure it from
    ...
    <p>There is no guarantee what specific values of <code>AMOUNT</code> exactly mean, as this depends on your graphic card capabilities.Just test each method and decide which looks best on your system and is fast enough.</p></dd>
