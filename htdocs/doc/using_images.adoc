...
1. When the image is part of a 2D user-interface, use cgeref:TCastleImageControl[]. Set the cgeref:TCastleImageControl.URL[] to load the image.
+
...
2. When the image is part of the viewport, use cgeref:TCastleImageTransform[]. Set the cgeref:TCastleImageTransform.Url[] to load the image.
+
...
3. The last method is to load the image to cgeref:TDrawableImage[]. This is the low-level approach, where you will need to manually handle drawing the image at the appropriate moment, following https://castle-engine.io/manual_2d_ui_custom_drawn.php[manual about custom-drawing UI things in CGE].
+
