...
+
In our engine, in addition to the standard X3D nodes, we also define a few _extension nodes_. E.g. we add nodes to make link:shaders[shader effects], link:screen_effects[screen (post-processing) effects], shadows and more.
...
In our engine, the first point is the most important: *X3D nodes are the core building block of everything*. That is, everything you see loaded into cgeref:TCastleScene[] is a graph of X3D nodes. All link:model_formats[3D and 2D model formats] are loaded into a graph of X3D nodes. You have Pascal API available to manipulate these nodes, to create and modify them at runtime.

== Examples

See our link:demo_models[demo models] for example X3D models.
