...
- cgeref:TEffectNode[] and cgeref:TEffectPartNode[] are both link:x3d[X3D] nodes. Don't worry, we explain everything it implies on this page.
...
2. To free the node, you could just call `FreeAndNil(Effect)`. But this is dangerous. What if something still refers to it, e.g. some scene? The recommended way is to call cgeref:TX3DNode.KeepExistingEnd[] to "return the control to the reference counting mechanism". And then do cgeref:FreeIfUnusedAndNil[] to free the node if it is no longer used by anything else _right now_. For example:
+
[source,pascal]
----
