...
First, create a simple _Castle Game Engine_ application with a link:viewport_and_scenes[TCastleScene inside a TCastleViewport] that shows some model (from any link:model_formats[supported format], like link:gltf[glTF] or link:x3d[X3D]).
