...
NOTE: _VS Codium_ users: The https://open-vsx.org/extension/theangryepicbanana/language-pascal[Pascal magic extension is also available in the VS Codium_marketplace].
...
* _"Castle Game Engine"_ extension, in both _VS Code_ and _VS Codium_ extensions marketplaces. Features:
+
--
...
- Remove unnecessary settings (current https://github.com/genericptr/pasls-vscode[pasls-vscode] that we recommend has a few settings that are actually ignored by our LSP server).
