...
NOTE: The configuration described here also works with https://github.com/VSCodium/vscodium[VSCodium], a version of VS Code built using 100% open-source code (without e.g. Microsoft telemetry). TODO: Our VS Code _"Castle Game Engine"_ extension will be uploaded to the _VS Codium_ marketplace soon.
...
You can follow this link. Or just search for _"Castle Game Engine"_ in the _VS Code_ extensions sidebar.

== Extension Settings and Pascal code completion

After installing https://marketplace.visualstudio.com/items?itemName=castle-engine-team.castle-engine[Castle Game Engine in VS Code] extension, go to the extension settings and configure engine, FPC and Lazarus paths to make it work:

cgeimg::block[vscode_extension_config.png|VS Code Extension configuration]

Notes about extension settings:

- Set `Engine Path` to point to our CGE directory. Inside CGE we expect to find `bin` subdirectory with `pasls` (`pasls.exe` on Windows).

- Set `PP` to your FPC executable. If you use FPC bundled with CGE, this should look like `<cgepath>/tools/contrib/fpc/bin/fpc-cge`. Add `.exe` extension on Windows. If you use regular FPC, this is a path to your regular FPC executable (`fpc`, `fpc.exe`, `fpc.sh`).

- Set `FPCDIR` to point to the FPC source code. If you use FPC bundled with CGE, this should look like `<cgepath>/tools/contrib/fpc/src/`.

- Set `LAZARUSDIR` to your Lazarus (source) directory. This is optional, fill it if you have https://www.lazarus-ide.org/[Lazarus] installed. It will enable to find LCL units and do the code completion for LCL components too.

- Set `FPCTARGET` to your OS, like `linux` or `win64`.
+
[NOTE]
====
...
====
+
- Set `FPCTARGETCPU` to your CPU, like `x86_64`.
+
[NOTE]
====
...
Setting up everything means that our _LSP server_ that implements an _intelligent code completion_ for Pascal and is aware of _Castle Game Engine_ units and default syntax options (e.g. FPC _ObjFpc_ mode) will work.
...
WARNING: TODO: This doc section is super-seeded by the new extension, that automatically gives commands to build and run for you.
...
WARNING: TODO: This doc section is super-seeded by the new extension, that automatically sets keys for you.
...
WARNING: TODO: This doc section is super-seeded by the new extension, that automatically gives `castleDebug` debugger for you, using `fpDebug` under the hood.

CGE applications compiled in _debug_ mode contain debug symbols compatible with GDB. Any debugger extension in _VS Code_ based on GDB should work, like https://marketplace.visualstudio.com/items?itemName=webfreak.debug[Native Debug].
