...
The above sample `tasks.json` assumes that the CGE link:build_tool[build tool] called `castle-engine` can be found on `PATH`. Make sure your environment is set correcttly to make it work:

- Extend the `PATH` environment variable to contain the `bin` subdirectory of your CGE directory. This is where `castle-editor[.exe]` file should be. _Alternative_: Edit the sample `tasks.json` to provide full path to the build tool exe.

- Make sure that compiler (FPC or Delphi) is on `PATH`, so that our build tool can find it.

- _Optional, should not be needed in the usual case:_ Set `CASTLE_ENGINE_PATH` environment variable to point to your CGE directory. This should not be needed: build tool should detect CGE directory anyway.
