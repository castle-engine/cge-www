# Visual Studio Code
include::common.adoc[]
:description: Castle Game Engine extension for Visual Studio Code (VS Code) and VS Codium, that provides code completion and highlighting for Pascal and management of Castle Game Engine projects.
...
We provide an extension that makes it easy to use _VS Code_ with _Castle Game Engine_ and Pascal:

* *Build, run, debug* (and more) _Castle Game Engine_ projects from VS Code.

* Code in Pascal with *syntax highlighting* and *code completion*.

It's easiest to install our extension by just searching for _"Castle Game Engine"_ in the _VS Code_ extensions panel.

- Or visit https://marketplace.visualstudio.com/items?itemName=castle-engine-team.castle-engine[extension page in the VS Code marketplace].
- The extension is also available for https://vscodium.com/[VS Codium] through the https://open-vsx.org/extension/castle-engine-team/castle-engine[Open VSX marketplace].

For the _code completion_, the extension relies that you have _Castle Game Engine_ installed. It will use the `pasls` tool (LSP server) that is part of the engine. The code completion works in any Pascal code -- not necessarily _Castle Game Engine_ projects. The Pascal LSP is also useful from other text editors (like _Emacs_ or _Vim_) to make _intelligent code completion for any Pascal code_.
...
https://vscodium.com/[VS Codium] users can find our extension in the https://open-vsx.org/extension/castle-engine-team/castle-engine[Open VSX marketplace].

== Extension Settings and Pascal code completion

Go to the extension settings and configure it:
...
+
The workaround for now is just to hide our buttons on the status bar, if they take too much space for you (right click on the status bar to customize what is shown there).
...
+
If you just install (unpack) CGE you could have a ready setup with CGE + VS Code. This is practically just _VS Code_ with automatically enabled (or suggested?) _"Castle Game Engine"_ extension mentioned above.
+
...
Plan resigned now -- people should also have a choice, given VS Codium, and given other forks.
////
