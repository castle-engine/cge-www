...
This Docker image can be used by continuous integration systems like link:woodpecker[Woodpecker CI] and link:github_actions[GitHub Actions]. It can also be used on your local computer. Instead of downloading FPC, CGE, Android SDK and NDK etc., you only have to get the Docker image.
...
The bash script, Dockerfiles, CI workflows etc. to generate these Docker images are open-source on https://github.com/castle-engine/castle-engine-docker .
