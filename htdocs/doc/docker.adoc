...
- `cge-none-fpc331` has recent FPC 3.3.1. The exact FPC 3.3.1 revision is listed https://github.com/castle-engine/castle-engine-cloud-builds-tools/blob/master/Dockerfile.no-cge#L249[here].
...
Note that the "alternative FPC versions" images come without CGE included (only all CGE prerequisites). Follow https://castle-engine.io/compiling_from_source.php[compiling from source] docs to set up CGE inside the container. If this sounds complicated, then use instead the tags `cge-stable` or `cge-unstable` with stable FPC.
