...
## Future: KTX (saving, for now we can only load them)

link:ktx[KTX] and link:dds[DDS] also offer some features similar to the `.castle-image` file format (fast load/save, at the expense of large file size).

And they are standardized (supported by other software) and link:ktx[KTX] ecosystem is growing and active.

As such, we plan to focus on perfect link:ktx[KTX] support and it may make some of the `.castle-image` advantages obsolete.