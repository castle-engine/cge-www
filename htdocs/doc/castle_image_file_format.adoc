...
WARNING: This format is to be removed soon. link:ktx[KTX] is now better in every practical regard, it also (almost) always results in super-optimal loading and saving using just 1 call to `Stream.ReadBuffer`. And KTX is standardized (supported by other software).
