...
A backend useful on most Unix systems, like _Linux_ or _FreeBSD_, that use X11. Tested also on X inside Wayland.
...
* Full-screen (cgeref:TCastleWindow.FullScreen[]) is realized using modern window manager features. This means we use `_NET_WM_STATE_FULLSCREEN` from https://specifications.freedesktop.org/wm/latest/[Extended Window Manager Hints].
+
[NOTE]
====
If the window manager doesn't support the modern hints, we fall back to an obsolete method using  `override_redirect`. This obsolete approach doesn't allow to switch applications with Alt+Tab from  fullscreen window, and will not handle keys under Wayland. This really should not happen on modern desktops, X or Wayland.
====
