...
A _Castle Game Engine_ project is a directory containing the link:project_manifest[CastleEngineManifest.xml] file. Most often it also contains a subdirectory `data/` that is accessed from code using the link:manual_data_directory.php[castle-data:/xxx] URLs. Everything else is up to you, organize your source code and data however you like.
...
cgeimg::block[
  ...
  vscode_completion.png|VS Code completion
]
...
* Note that the `data/` subdirectory is somewhat special. It is automatically detected, it is automatically packaged (e.g. in Android apk), and it can always be accessed by link:https://castle-engine.io/manual_data_directory.php[castle-data:/xxx URL]. You will place there 3D models, 2D images, designs (`xxx.castle-user-interface`, `xxx.castle-transform`, `xxx.castle-component` files) and everything else you load in the game.
+
...
+
--
* On 3D and 2D models we run link:view3dscene.php[view3dscene].
* On images we run link:castle-view-image.php[castle-view-image].
...
* Drag audio files on `TCastleViewport` to automatically create link:manual_sound.php[sound source (TCastleSoundSource and TCastleSound)].
