...
+
We recommend link:download#delphinus[installing Castle Game Engine into Delphi IDE using Delphinus]. This will install our design-time packages, components, and extend your Delphi units path.
+
Alternatively, you can do it manually.
+
--
- Open in Delphi package `packages/delphi/castle_engine.dproj` (from CGE directory) and install it (right-click on the `castle_engine.bpl` and choose _"Install"_ from the context menu).

- If you create a new project (not only compile CGE examples) remember to set proper paths to CGE units. You can add these paths to each project or to the global Delphi settings. Copy the paths from one of our examples, like `examples/delphi/fmx`.
--
