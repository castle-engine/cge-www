...
+
We recommend now that you link:download#delphinus[install Castle Game Engine into Delphi IDE using Delphinus]. This will install our design-time packages, components, and enhance your Delphi units path.
+
Alternatively, you can do it manually. Install packages like this:
+
--
...
5. If you will create new projects (not only compile CGE examples) remember to set proper paths to CGE units. You can add these paths to each project or to global Delphi settings. Copy the paths from one of our examples, like `examples/delphi/fmx`.
--
