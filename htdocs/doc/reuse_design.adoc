...
A related engine feature is cgeref:TCastleComponentFactory[]. Use this to instantiate the same `.castle-user-interface` or `.castle-transform` multiple times. For example, you could use this to:
...
- https://github.com/castle-engine/castle-engine/tree/skinned-animation-gpu/examples/2d_games/space_shooter[examples/2d_games/space_shooter] (spawning bullets, rocks, explosions)

- https://github.com/castle-engine/castle-engine/tree/master/examples/animations/animate_bones_by_code[examples/animations/animate_bones_by_code] (spawning characters for _"Add Crowd"_ test).

From code, the primary method to use is cgeref:TCastleComponentFactory.ComponentLoad[]. It creates another instance of given design, that you can add to the hierarchy.

NOTE: Hint: if you have scenes inside the designs, that are instantiated many times, usually setting cgeref:TCastleSceneCore.Cache[] to `true` is beneficial. It will avoid loading the scene from disk multiple times.