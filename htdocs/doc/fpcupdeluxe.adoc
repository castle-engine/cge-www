...
* The engine tools, in particular link:build_tool[build tool] and link:editor[editor] are not compiled for you. You have to compile them yourself, following link:compiling_from_source[compiling from sources] documentation. *We don't recommend attempting this to new CGE users, it is easier to link:/[just download CGE binary release from our page].*
