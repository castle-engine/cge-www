...
* The API documentation is not present in the `doc/reference/` , because we don't keep the generated docs inside the repository. That's usually not a problem, as you can https://castle-engine.io/apidoc-unstable/html/[browse the Castle Game Engine API docs online]. You can also get link:pass:[https://github.com/pasdoc/pasdoc/wiki][pasdoc] and generate them yourself by `make html` in `doc/pasdoc`.
