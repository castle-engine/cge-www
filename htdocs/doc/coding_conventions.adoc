...
- `EXTENDED_EQUALS_DOUBLE` symbol when `Extended` is 8-byte `Double`. Useful to not define overloads for both `Double` and `Extended` -- they would be equivalent (thus, not allowed by compiler) when these types are equal.
