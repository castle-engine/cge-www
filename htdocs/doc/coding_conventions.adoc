...
To make things simple, all future code should just talk about URLs (not URIs), and our https://castle-engine.io/manual_network.php[documentation about URLs] just clearly says _"most URLs actually accept a URI (a bit broader term than URL); in particular you can use https://en.wikipedia.org/wiki/Data_URI_scheme[data URI scheme]"_.
...
- rerun `castle-engine generate-program --guid-from-name` in some applications in CGE that require updated DPROJ, but are not affected by `examples/regenerate_auto_files_in_all_examples.sh`:
