...
Use this shebang (first line of a shell script):
...
This shebang is more portable than `#!/bin/bash` -- as it works reliably also on FreeBSD and OpenBSD. In total, we want our bash scripts to work on a variety of systems: Linux, FreeBSD, macOS, Windows (with Cygwin or MSYS2) etc.
...
For historical reasons, some of our scripts may use something a little more relaxed, like
...
Usually is just a historical thing. We should follow http://redsymbol.net/articles/unofficial-bash-strict-mode/[Use Bash Strict Mode] fully going forward (except shebang, which we want more portable).
