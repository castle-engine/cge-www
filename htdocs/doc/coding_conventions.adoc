...
This guideline is thus: *When you create an instance of something, then you decide when and how it should be freed. Nothing else should free it, unless explicitly permitted by the documentation.*a
...
==== If you receive an instance from some code you don't control, do not assume anything about the lifetime of this instance
...
In Pascal, a value of an object instance (like `X: TMyObject`) can always be `nil`.

This guideline limits the cases when you have to worry that something is `nil`, in order to avoid writing lots of checks `X <> nil` in the code.
