...
WARNING: We don't yet support https://wiki.freepascal.org/FPC_Unicode_RTL[FPC with Unicode RTL]. But it will be, and the behavior will be then equal to Delphi behavior.
...
Our unit cgeref:CastleUnicode[] provides simple _Unicode-safe_  API to deal with strings that works with both FPC and Delphi painlessly. In particular, use:
...
to use 8-bit string. And use `AnsiChar` for 8-bit character, and `PAnsiChar` to have a pointer to them.

And you can use `UnicodeString` for UTF-16 encoded strings. (And `WideString` for weird non-ref-counted version.) With `WideChar` and `PWideChar` to have a pointer to them.

When to use them:

- Use `Utf8String` to explicitly mean _"8-bit string, with UTF-8 encoding"_.
+
One example when we use it is for API to read / write to streams (see cgeref:CastleClassUtils[] routines). Reason: UTF-8 is the file format that most software expects, and it is a superset of ASCII (that is: simplest text files).
+
( The exceptional routines that _read / write to streams in different format depending on compiler (they read/write 8-bit on FPC and 16-bit on Delphi)_ are marked with `DefaultString` in the name. Right now, the only such routine is cgeref:MemoryStreamLoadFromDefaultString[]. As said above, we prefer to have most routines use UTF-8 for text files. )
+
Another example when explicit `Utf8String` type makes sense is when dealing with external libraries that specifically require 8-bit strings with UTF-8 encoding. Like link:steam[Steam].
...
- *Seldom needed*: Use `UnicodeString` or `WideString` when dealing with native APIs that use UTF-16 encoded strings, like WinAPI. Prefer `UnicodeString` as it is ref-counted and consistent with `AnsiString`.

See https://github.com/castle-engine/castle-engine/blob/ansistring-encoding-configurable/doc/miscellaneous_notes/ansistring_encoding.md[CASTLE_ANSISTRING_xxx symbols, AnsiString vs Utf8String in Castle Game Engine] for more information on the usage of different string types in _Castle Game Engine_.
