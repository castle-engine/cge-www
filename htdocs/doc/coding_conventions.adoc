...
NOTE: We don't have a strict rule for the order of units within each group. They _often_ loosely follow the "dependency order" (if unit `A` is used by `B`, then we write `A, B`, like `SysUtils, Classes, Controls`) but we really don't make this a strict rule (it would be too tiresome to maintain, for no gain). Many `uses` clauses have in practice an order dictated by history and that's OK.
...
=== You can (and should) free the things you allocated; you cannot free other things, unless explicitly allowed
...
It's often quite easy to manage this, _assuming that other code "plays nice"_:

1. If you create something in your _constructor_, then you are responsible for destroying it in your _destructor_ (or when it needs to be recreated for some reason).
+
This works OK but _assumes that nothing else will free your instance_. Otherwise you will get a _dangling pointer_, i.e. a non-`nil` reference that is invalid because something else freed the memory it pointed to.
+
_Some_ code may allow others to free the things it created. It can avoid having a dangling pointer e.g. by observing the reference using `FreeNotification` or cgeref:TFreeNotificationObserver[], see link:custom_components[custom components]. (Or by not storing the reference at all.) But this permission ("you can free this") has to be explicitly documented as allowed.
...
+
_Other things cannot free the children, or you will get dangling pointers._
...
+
_Other things can still free it, this will be handled OK._
--

In short, things are easy when _nothing else frees the object you were supposed to free_.

This guideline is thus: _When you create an instance of something, then you decide when and how it should be freed._ Nothing else should free it, unless explicitly permitted by the documentation.

This includes the cases when you receive an instance of something by accessing some property, function or class function. _Do not_ free it, unless it is explicitly documented that you should free it. Only free things where you have called `constructor`.

=== What can be nil

In Pascal, a value of an object instance (like `X: TMyObject`) can always be `nil`. But the code that does a lot of `X <> nil` comparisons at every step is hard to read, so we want to limit usage of `nil`.

To make things easy, follow this:

- All the object instances given as routine parameters must be non-`nil`. (That is, they cannot be `nil`.) This puts the burden on the caller, while the implementation of a routine can assume they are not `nil`.
+
If a particular routine allows some parameter as `nil`, it should be explicitly documented.
+
...
- All the functions that return an object instance always return a non-`nil` value. This puts the burden on the function implementation, not on the caller.
+
If a particular routine can return `nil`, it should be explicitly documented.
+
...
- All the properties of an object instance are non-`nil` value (after the object is created).
+
In a property may be `nil`, it should be explicitly documented.
+
