...
+
To fix this, perform one more step: disable GPU skinning by setting
+
[source,pascal]
----
TSkinNode.InternalFeatures.Shaders := false;
----
+
early in your program. Use the cgeref:X3DNodes[] unit to have cgeref:TSkinNode[] identifier available.
+
For example, you can do this from your `Application.OnInitialize` callback. Typically this is a procedure `ApplicationInitialize` in your `gameinitialize.pas` unit. More generally: do this anytime after `ApplicationProperties.OnGLContextEarlyOpen` has run but before your model actually starts animating.

3. To make this work, you have to use ray-casts that don't depend on physics engine, and set cgeref:TCastleSceneCore.PreciseCollisions[] to `true` on the given scene. Otherwise, the entire scene will behave as a single box for collisions.
