...
- Camera transformations can be animated too. The advised approach to do this is to place camera as a child of animated bone, exposed using cgeref:TCastleSceneCore.ExposeTransforms[TCastleScene.ExposeTransforms]. See https://www.youtube.com/watch?v=gwnKMnAGUcg[Viewports, cameras and navigation in Castle Game Engine movie] for a demonstration.
...
This is available using cgeref:TCastleSceneCore.ExposeTransforms[TCastleScene.ExposeTransforms]. You can "expose" a bone transformation as `TCastleTransform` child and attach there a scene. See https://castle-engine.io/wp/2020/10/09/attach-objects-to-animated-bones-like-weapon-in-characters-hand-by-exposetransforms/ .
...
Demo of this technique is in https://github.com/michaliskambi/x3d-tests/tree/master/gltf/avocado_and_exports . Open the models there with https://castle-engine.io/castle-model-viewer[Castle Model Viewer] or just load them into `TCastleScene` instance in CGE editor.
...
[#unique_names]
=== glTF files may have non-unique names, but we advise to keep them unique, and we'll force them unique at loading
