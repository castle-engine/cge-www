...
We absolutely recommend using it. We focus on supporting this format perfectly in _Castle Game Engine_, with all the features and efficiency. It is supported by a lot of tools, e.g. you can export to it from link:blender[Blender] easily.

== Loading glTF

Simply use cgeref:TCastleScene[] to load a glTF model. See link:viewport_and_scenes[Viewport with scenes, camera, navigation] for all details.

The rest of this page describes details about glTF support in _Castle Game Engine_.
...
- https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_node_visibility[KHR_node_visibility] - toggle visibility of nodes (transformations, groups).
--

=== TODO

- The most important missing glTF feature: morph targets. It is coming!

- https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_animation_pointer[KHR_animation_pointer]. Just like with X3D "routes", this allows to animate almost every part of the model.

- link:roadmap#gltf_meshes_gpu[More efficient glTF loading, straight to GPU buffers].

- link:roadmap#gltf_material_features[glTF material extensions].
...
For speed, the shapes animated using _skinned animation_ in glTF use bounding box for collisions. That's because the triangles would change every frame and updating the octree would have a significant cost for FPS.

If you need to have more precise collision detection:

=== Recommended: Attach physical colliders to exposes joints transformations

We recommend to use the cgeref:TCastleSceneCore.ExposeTransforms[] mechanism documented in link:expose[expose scene elements] manual chapter.

. _Expose_ the joints you're interested in by adding them to cgeref:TCastleSceneCore.ExposeTransforms[]. E.g. expose a head, if you want to detect headshots. This will result in having a cgeref:TCastleTransform[] representing this joint.

. Attach link:physics[physical colliders] to these joints. For example, attach cgeref:TCastleSphereCollider[] to the head joint. Remember to also attach cgeref:TCastleRigidBody[] to the joint and set cgeref:TCastleRigidBody.Dynamic[] to `false` and cgeref:TCastleRigidBody.Animated[] to `true`. This means your joint transformation is determined by the animation.

. Then use physics-based collision resolution, e.g. calling cgeref:TCastleAbstractRootTransform.PhysicsRayCast[MyViewport.Items.PhysicsRayCast].

This is recommended as everything remains fast, and we delegate collision detection to the link:physics[physics engine], which is well optimized for this.

=== Alternative solution: Use an alternative model for collisions
...
NOTE: If you use ray-casts that don't depend on physics engine, remember to also set cgeref:TCastleSceneCore.PreciseCollisions[] to `true` on the `mycreature.x3dv` scene.

=== Alternative solution: Force recalculating precise octree every frame

If you desperately need precise collisions, per-triangle, with animated models (so the collision structure has to be updated at runtime), and you can tolerate some performance loss (it may be acceptable for smaller models) then follow this solution.

1. You can find all cgeref:TAbstractShapeNode[] occurrences in the model, and change the cgeref:TAbstractShapeNode.Collision[] from cgeref:scBox[] to cgeref:scDefault[].
+
Like this:
+
[source,pascal]
...
begin
  ...
  Model.EnumerateNodes(TAbstractShapeNode,
    {$ifdef FPC}@{$endif} ForceShapePrecise, false);
  Scene.Load(Model, true);
end;

procedure TMyView.ForceShapePrecise(Node: TX3DNode);
begin
  (Node as TAbstractShapeNode).Collision := scDefault;
end;
----

2. Moreover, we perform link:skin[skinning using GPU shaders by default]. This means that the above step (switching cgeref:TAbstractShapeNode.Collision[] to cgeref:scDefault[]) will make us use precise collisions, but they will be resolved using the _"rest pose"_ of the model, not the current animated pose.
+
To fix this, perform one more step: disable GPU skinning by setting cgeref:TCastleRenderOptions.SkinnedAnimationShaders[] to `false` on the given scene. You can do this by Pascal code or by just tweaking the `MyScene.RenderOptions.SkinnedAnimationShaders` in the editor.

3. To make this work, you also have to use ray-casts that don't depend on physics engine, and set cgeref:TCastleSceneCore.PreciseCollisions[] to `true` on the given scene. Otherwise, the entire scene will behave as a single box for collisions.
...
For example of this technique see https://github.com/castle-engine/demo-models/tree/master/animation/blender_skinned_animation , in particular file https://github.com/castle-engine/demo-models/blob/master/animation/blender_skinned_animation/skinned_anim_run_animations_from_x3d.x3dv[skinned_anim_run_animations_from_x3d.x3dv] there. The X3D file has this code:
...
Demo of this technique is in https://github.com/michaliskambi/x3d-tests/tree/master/gltf/avocado_and_exports . Open the models there with link:castle-model-viewer[Castle Model Viewer] or just load them into cgeref:TCastleScene[] instance in CGE editor.
