...
`Jenkinsfile` is a file that you should commit to your repository and it will configure the build job.
...
We maintain a https://jenkins.castle-engine.io/[Jenkins for Castle Game Engine] server. It is used for _Castle Game Engine_ development, running a lot of CGE tests and builds on various slaves (Linux, Windows, macOS, Raspberry Pi). Michalis grants access to it. If you are a regular CGE contributor and you would like such access, mailto:michalis@castle-engine.io[send an email to Michalis Kamburelis].
