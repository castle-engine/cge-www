...
. Initially the list of packages in _Delphinus_ will be empty. You have to refresh them, and for this you need a GitHub _OAuth-Token_.

. To create a token, login to your https://github.com/[GitHub] account and visit https://github.com/settings/tokens[Developer settings -> Personal access tokens -> Tokens (classic)]. Choose _"Generate new token (classic)"_.
...
. Test it! Navigate to xxx and open some CGE example, like `x`
