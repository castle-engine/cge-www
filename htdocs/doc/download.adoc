...
+
--
- If you installed CGE from zip, or link:compiling_from_source.php[from sources], the same instructions apply. Just point Delphinus to wherever the CGE is installed. It will have a file `Delphinus.Install.json`.

- Installation will fail if you have previously installed Delphi packages manually. Remove them first using Delphi _"Component -> Install Packages..."_ dialog (despite the name, this dialog allows also to remove packages).

- After upgrading CGE, repeat this process to install in Delphi the latest versions of the packages.
--
...
. Now find _"Castle Game Engine"_ in Delphinus, click to install it. Choose version as you see fit (right now only `HEAD` is available). The final CGE directory is like `C:\Users\Public\Documents\Embarcadero\Studio\<VERSION>\Comps\castle-engine\` (look at _"Show Log"_ when installing to see the exact path).
+
