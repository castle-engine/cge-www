...
=== GitHub releases with more variants
...
- Alternative zip, 7z, tar.xz versions.
...
=== Alternative downloads at SourceForge

We maintain a mirror of our releases at https://sourceforge.net/projects/castle-engine/files/castle_game_engine/[SourceForge (files of castle-engine project)]. You can download from there the same files as from GitHub releases.

Why? We found cases of networks where downloads from https://github.com/castle-engine/castle-engine/releases[GitHub releases] don't work reliably (are often interrupted in the middle) while downloads from SourceForge behave reliably. Your mileage may vary -- you have an option.
...
+
--
- Be sure to check _"Show Log"_ at the end, to make sure installation was successful. At the end the log should say _"Installation finished"_, not _"Error: Installation failed"_. If you encounter a problem, please consult the messages above.

- If you installed CGE from zip, or link:compiling_from_source[from sources], the same instructions apply. Just point Delphinus to the `Delphinus.Install.json` file in the top-level CGE directory.
...
We also encourage to build and run some CGE examples, like `examples/3d_games/explore_impressive_castle` and `examples/delphi/fmx`. Though the examples would build fine even without using Delphinus to install (as their DPROJ files already contain all necessary paths).

NOTE: In order to run some examples on Windows, you will need to place DLL files alongside the EXE file. Copy the DLL files from engine locations mentioned link:compiling_from_source#section_Windows[here]. Or compile the example at least once from link:editor[CGE editor], it will copy the DLL files automatically.
...
If you have to use an older system, you can still use CGE by link:compiling_from_source[compiling it from sources] to link to your current GLIBC. We plan to distribute the engine also as https://snapcraft.io/[snap package] in the future, which will sidestep this limitation.
...
[role="compact"]
. _OpenGL_
. _GTK 2_ and _GTK 3_ (install both versions now; our editor uses GTK 2 for now, but other tools and your games use GTK 3)
. _LibPng_ (_optional_; to open png files more efficiently)
. _ZLib_ (_optional_; to unpack gzip files; also used by LibPng)
. _OpenAL_ (_optional_; to play sound)
. _FreeType_ (_optional_; to load font files)
. _VorbisFile_ (_optional_; to load OggVorbis sound files)
. _Qt_ and _libQtPas_ (_very optional_; only if you want to compile our editor with alternative Qt widgetset)

Most of them are already present on all Unix desktop installations.

On your (developer) system, you will also need the _development_ versions of some of these libraries. This allows to build programs that link to these libraries.
...
----
+
(Anything missing? https://archlinux.org/packages/[Search])

* On _Void Linux_, try this:
+
[source,bash]
----
...
----
+
(Anything missing? https://voidlinux.org/packages/[Search])
...
This in particular applies to Windows. No matter how much you like your _Windows XP_, it's time to move on :), either to a newer Windows or to a different system, like Linux. As of 2026, https://en.wikipedia.org/wiki/List_of_Microsoft_Windows_versions[Windows 11 is the only Windows version with security support], though we temporarily support also _Windows 10_ as its widely used. But please be safe and upgrade!
...
+
Note that we're not alone in this approach. _Google Chrome_, _Firefox_, _Steam_, _GPU drivers_.... a lot of the software is not supported on older systems. Which means that if you use an old system, you're not only missing out on system updates, you also miss on updates to web browsers, drivers and other software. This implies even more security risks.
//And the risk that engine will fail to work for reasons unrelated to the engine itself.

// Our binaries are now signed by Azure, not SignPath.
////
== Code Signing Policy

WARNING: As of 2025-11-01, we apply for SignPath code signing, per the https://signpath.org/terms[terms]. The binaries available on this page are not _yet_ signed, but they will be, provided our application will be approved.

Free code signing provided by https://about.signpath.io/[SignPath.io], certificate by https://signpath.org/[SignPath Foundation].

Team:

* link:credits[Contributors]
* Project Lead: https://github.com/michaliskambi/[Michalis Kamburelis]

Our link:privacy_policy[Privacy Policy].
////