...
- To access the component from code, you need to declare it in the `published` section of your class, like `MyLabel: TCastleLabel`. The field `MyLabel` will then be automatically initialized when state starts to point to the component in the design with the name `MyLabel`. You can modify this component from anywhere you want, e.g. doing `MyLabel.Caption := 'something'`.
