...
NOTE: We could change names of these uniforms / attributes, when generating shaders for MaterialX. But there's no point -- we need to adjust to _types_ of these uniforms / attributes anyway, _and_ using default MaterialX names means we have names that are compatible with all X3D browsers (no more `castle_ModelViewMatrix`, `freewrl_ModelViewMatrix` etc.). So our best course of action is to just follow exactly the default MaterialX uniform / attribute names. We could even invent a boolean like `SFBool materialXVariableNaming` to enable using such naming for all custom shaders, whether MaterialX or not -- as this would standardize shader variables across X3D browsers.
