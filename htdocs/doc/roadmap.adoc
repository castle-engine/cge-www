...
### (IN PROGRESS - ALMOST DONE) Finish TCastleMoveAttack behavior
...
+
--
- https://github.com/castle-engine/castle-engine/tree/master/examples/creature_behaviors[examples/creature_behaviors]
- https://github.com/castle-engine/castle-engine/tree/master/examples/3d_games/explore_impressive_castle[examples/3d_games/explore_impressive_castle]
- https://github.com/castle-engine/castle-engine/tree/master/examples/3d_games/dungeons_and_skeletons_fight[examples/3d_games/dungeons_and_skeletons_fight]
--
...
Activating link:shadow_maps[shadow maps] has some disadvantages now:
...
+
See https://forum.castle-engine.io/t/not-all-objects-can-receive-shadow-map/2064/2[this forum thread] for example testcase.
...
- Moreover: `KHR_materials_retroreflection`:
+
--
- Example: https://create3000.github.io/x_ite/laboratory/gltf-sample-viewer/?url=TrafficCone
- `RetroreflectionMaterialExtension` node of X_ITE: https://create3000.github.io/x_ite/components/x-ite/retroreflectionmaterialextension/
- Draft of glTF extension: https://github.com/mklefrancois/glTF/tree/c33751d337d98e2d2b9f48e51cff9efa4d487b8b/extensions/2.0/Khronos/KHR_materials_retroreflection
--
...
TODO: Also read: https://github.com/TokisanGames/Terrain3D

[#vulkan]
### Vulkan renderer (maybe; alternative: use BGFX to get Vulkan)
...
+
It's similar to how we already have "sound backends" (OpenAL, FMOD, Nintendo Switch-specific, SOX for testing), and "game controllers backend" (WinAPI version, Linux version, Nintendo Switch version). We also have link:castlewindow_backends[TCastleWindow backends] though these are internally realized in a different way, which I no longer like and some day will have to change :)
...
+
So in practice, I will likely start doing BGFX by playing with `new_renderer_skeleton` example, to get quick results, and learn BGFX API. Once I'll confirm this is useful route for us, and I have a demo "CGE rendering using BGFX", then I can introduce the "renderer backend" and make BGFX rendering an alternative backend. This way I can first confirm that BGFX rocks :), and learn BGFX, before committing to larger rework to integrate it with CGE properly.

[#opengles_wirefame]
### (IN PROGRESS) Mobile: Support TCastleRenderOptions.WireframeEffect on OpenGLES
...
### macOS: Package engine as one, big, signed application bundle

This will:

- Be simpler to users, as the engine will be "one single application" for macOS users.

- Allow to run CGE editor on macOS by just double-clicking it, out-of-the-box. No translocation or other Apple shenanigans.
...
We already have infrastructure to sign macOS applications, see https://github.com/castle-engine/castle-build-ci/tree/master/apple[castle-build-ci/apple scripts].
...
MaterialX is a standard way to express shaders: https://materialx.org/

NOTE: Thanks go to _Aaron Bergstrom_, author of https://github.com/und-dream-lab/rawkee[RawKee (modern exporter from Maya to X3D)] for research on this!

Use-case:

- We can design complex shaders in authoring tools like link:maya[Maya] (possible with latest Maya), maybe link:blender[Blender] too?, that already have visual shader node editors.

- Possibly we can make 3D modeling of realistically-looking stuff easier by reusing libraries of realistic materials?

_Providing uniforms and attributes to shaders generated for MaterialX:_ This implies a few trivial changes to our shader usage code, we would provide uniforms and attributes with standard names like this:

- Attribute `i_position`, `vec3` -> Object-space vertex position
- Uniform `u_worldMatrix`, `mat4` -> Object-to-world transform
- Uniform `u_viewPosition`, `vec3` -> Camera position in world space
- and much more, following
+
--
...
- https://materialx.org/docs/api/_hw_shader_generator_8h_source.html
--
- The light source data is expressed using `u_lightData[]` with per-light structure build dynamically depending on the light type. Typical light sources are defined in https://github.com/AcademySoftwareFoundation/MaterialX/blob/main/libraries/lights/lights_defs.mtlx .

We would initially use the MaterialX naming convention when:

- We have `PackagedShader` with MaterialX file
- Or `ComposedShader` with GLSL with `SFBool materialXVariableNaming` set to `TRUE`.

The latter means that if exporter already generates MaterialX->GLSL during the export phase, we will be able to consume the resulting GLSL.

NOTE: Maybe even the `SFBool materialXVariableNaming` should be by default `TRUE`. So we follow MaterialX naming by default for custom shaders. This is nice -- as it means we have cross-browser compatibility for these things, every X3D browser would be expected to provide data to shaders using the same names/types of uniforms/attributes. No more dealing with `castle_ModelViewMatrix` vs `freewrl_ModelViewMatrix` etc. for browser authors.

### MDL integration?

See https://developer.nvidia.com/rendering-technologies/mdl-sdk .

Unsure if it makes sense -- MaterialX integration seems filling the same space, and it is open standard, and close to OpenUSD too.
...
Address a few shortcomings of our  (see `examples/screen_effects_demo/`, link:screen_effects[screen effects]):
...
[#gltf_save]
### (IN PROGRESS) Export (save) nodes graph to glTF

Currently we can export (save) nodes graph (from one cgeref:TCastleScene[] or, using `TCastleTransform.InternalBuildNode`, from whole cgeref:TCastleViewport[] contents) to X3D or STL.
...
NOTE: We've made a start toward this, specifically for link:skin[skinned animation], in internal `TCastleSceneInstance` (right now doesn't render, but maintains customized state and can calculate bbox).
...
[#expose_to_code]
### Expose component name for Pascal code and make sure `uses` clause pulls all units to register components in the design
...
+
NOTE: This is actually independent from whether the component is listed in the `published` section or not. The unit defining it must be listed in the `uses` clause, to register it and be able to deserialize it.
+
NOTE: Which unit should have the `uses` clause extended? `GameInitialize`, `CastleAutoGenerated`, or particular view's unit like `GameViewXxx`? The last option (`GameViewXxx`) is best, so that, if the view is not used in the application, then also the dependencies are not pulled. This is also most similar to what Delphi/Lazarus do for forms.
...
+
--
- Get link:compiling_from_source[Castle Game Engine sources and learn how to build editor yourself].
...
Already merged PR: https://github.com/castle-engine/castle-engine/pull/636

Next step: make it exposed in model viewer / converter, see https://github.com/castle-engine/castle-model-viewer/compare/master...url-processing[url-processing branch in castle-model-viewer].
...
- to enable you to use cool CGE features like link:mirror_plane[mirrors], link:shaders[shader effects] easier.
...
NOTE: This feature has been demoted from "must have for 7.0" to "we can do it after 7.0". Because engine makes sense without it, we have to limit our scope for release at some point :), and our link:shaders[shader effects] got some updates (to API and docs) making them easier to use (which fills _some_ use-cases for new material components).
