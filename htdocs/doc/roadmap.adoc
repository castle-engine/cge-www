...
- to enable you to use cool CGE features like mirrors ( https://castle-engine.io/x3d_extensions_mirror_plane.php ), shader effects ( https://castle-engine.io/compositing_shaders.php ) easier.
...
More physics engine integration. The details are listed in the link:physics#_future_plans_todos[manual about physics, subsection "TODO"]. In short:
...
[#steam]
## (IN PROGRESS) Steam integration, and engine available as Steam app
...
_This is already in-progress by Eugene Loza (branch: https://github.com/castle-engine/castle-engine/commits/castle-steam ), we also have a draft application on Steam for CGE :)_.
...
[#shapes_across_scenes]
## Possibility to reorder/merge all shapes, across all scenes (better blending, better batching)
...
NOTE: We have `TGLRenderer.ViewportRenderBegin`, `TGLRenderer.ViewportRenderEnd`, these are a step in the right direction. But we can do much more.
...
+
This means that you no longer need to do link:blending[manual sorting] to have blending work correctly across scenes.
+
This means that `TCastleViewport.Items.BlendingSort` can be actually a robust solution covering all cases, and https://github.com/castle-engine/castle-engine/issues/463[GitHub #463] has a trivial answer (just set `TCastleViewport.Items.BlendingSort := bs2D`).
+
This means that link:blending[blending] should be a much shorter page, because `TCastleViewport.Items.BlendingSort` should really be everything you need.

- When cgeref:DynamicBatching[] is active, we can have one "batching renderer" instance for the viewport (so, reused by all scenes) and it can merge shapes even from different scenes.
+
This means that cgeref:DynamicBatching[] is much more powerful.
+
This means that rendering thousands of cgeref:TCastleImageTransform[] with the same URL is just 1 draw call.
+
...
For now: animation of anhyting is absolutely possible, *from code*. You can  update any values you want in `Update` and change any value you need (like `MyControl.Translation`) using routines like `Lerp` or `SmoothStep`.

The goal here is to allow designing and testing such animations in CGE editor. The use-case should cover animating anythign that you see in CGE editor directly: e.g. UI stuff (like `TCastleUserIterface.Translation`, `TCastleImageControl.Color`), 3D stuff (`TCastleTransform.Translation`, `TCastleTransform.Rotation` etc.).
...
## Allow animating selected graph properties, without changing the graph
...
The above is ignored, because `ImageControl.CustomShader` (equal to `ImageControl.Content.CustomShader`) overrides `TDrawableImage.CustomShader` it. Similarly a few other drawable image properties, as `TCastleImagePersistent.Draw` does now:
