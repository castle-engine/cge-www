...
### (IN REVIEW) New navigation components that support physics out-of-the-box
...
### Remove the need to care about PreciseCollisions, cleanup old collision detection using our octrees
...
- Make cgeref:TCastleSceneCore.PreciseCollisions[] always `true` in editor.

[#steam]
### (IN REVIEW) Steam integration, and engine available as Steam app

Official Steam integration in CGE.

NOTE: We already used it for The Unholy Society, but we want to have better approach, without an intermediate "wrapper" library.

We should also upload CGE as app to Steam.

This is already in-progress:

- branch: https://github.com/castle-engine/castle-engine/commits/castle-steam
- PR (by Eugene Loza): https://github.com/castle-engine/castle-engine/pull/471
- we also have a draft application on Steam for CGE

### (IN PROGRESS) Implement TCastleWalkAttack behavior, use for enemies in examples/fps_game and examples/creatures_behaviors

In progress.

- See https://github.com/castle-engine/castle-engine/blob/master/src/transform/castlebehaviors_moveattack.inc[castlebehaviors_moveattack.inc].
...
### Release mobile castle-model-viewer (as Android and iOS application)

link:https://github.com/castle-engine/castle-model-viewer-mobile[This is almost done, thanks to Jan Adamec.] We need to release it :)

Associated with glTF, X3D and other formats that the castle-model-viewer (and _Castle Game Engine_) handles. Available in the App Store / Google Play Store. For free or for 1 USD (not sure yet; but definitely without ads, I really dislike ads).
...
- Tools to plant big things (3D `TCastleScene`) in the terrain, like trees, rocks. You can do it now, though it's manual: Use behavior cgeref:TCastleStickToSurface[] and put multiple instances of the thing you want. Use cgeref:TCastleTransformReference[] to make it optimal for memory when you can, so that multiple instances share the same resources.
+
...
2. Caching right now sometimes "defeats" reloading. If something is in the cache and is referenced at least 2 times (e.g. there are 2 cgeref:TCastleImageTransform[] components referring to the same image), then reloading doesn't work.
+
