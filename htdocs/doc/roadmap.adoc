...
## Automatically reload (in editor) data files when they change

By deafult, at design-time, `TCastleScene` and other components loading stuff from URLs (possibly from `data`) should be notified when the file changes. And they should reload it automatically, without the need for user to do _"Reload URL"_ from context menu.

This should account for various changes:

- From CGE own sprite sheet editor.

- From external applications, e.g. exporting from Blender to glTF, changing Tiled TMX file, editing image (e.g. to add alpha bleeding using castle-image-view).

There are OS-level notifications for file changes. As a fallback we should use internal in-process notifications, that would at least work e.g. for changes done in sprite sheet editor.
