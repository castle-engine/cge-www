...
[#gltf_skinning_gpu]
### Efficient skinned animation on GPU (from glTF and more) using new SkinnedAnimation node
...
NOTE: This feature has been demoted from "must have for 7.0" to "we can do it after 7.0". Because engine makes sense without it and we have to limit our scope for release at some point :)
