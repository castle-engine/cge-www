...
This is started -- past `Spatial` property is now both simplified (only `ssRendering,ssDynamicCollisions`, does not manage other structures) and private. Only cgeref:TCastleSceneCore.PreciseCollisions[] is public.
...
### (IN PROGRESS) Distance field fonts
...
Currently you can only export `TCastleScene` to X3D or STL.
...
2. Caching right now sometimes "defeats" reloading. If something is in the cache and is referenced at least 2 times (e.g. there are 2 cgeref:TCastleImageTransform[] components referring to the same image or you use cgeref:TCastleScene.Cache[] and the same scene URL is loaded 2 times), then reloading doesn't work.
+
...
### Options to deal with resources (like textures) when saving
