...
+
If you see weird seams between tiles when using cgeref:TCastleTiledMap.SmoothScaling[] = `true`, this is likely a lack of spacing and link:alpha_bleeding[alpha bleeding] between tiles. You have to process your tileset image by adding some spacing between tiles and perform link:alpha_bleeding[alpha bleeding] in your image-editing software (you can use our link:castle-image-viewer[Castle Image Viewer]). Alternatively, you can activate cgeref:TCastleTiledMap.SmoothScalingSafeBorder[] or cgeref:TCastleTiledMap.ForceTilesetSpacing[] to workaround it.
