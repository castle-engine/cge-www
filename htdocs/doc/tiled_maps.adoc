...
- **Can we query the map position under an arbitrary UI position, not just where the mouse is?**
+
Sure. To do this, know that cgeref:TCastleViewport.MouseRayHit[MyViewport.MouseRayHit] is equivalent to:
+
--
...
- **Can we query the map position such that the map is not obscured (for this query) by stuff on top of the map, like NPCs on map?**
+
...
+
It is easiest when you have a parent `TCastleTransform` group that contains e.g. all NPCs. You can then hide them by setting e.g. `MyNpcs.Exists := false;` and then restore visibility by setting `MyNpcs.Exists := true;`. E.g.:
+
[source,pascal]
----
var
...
----
+
NOTE: Using the physics engine collision queries, you can utilize physics _layers_ to limit what is hit by the ray-cast. TODO: Extend our API and docs to enable physics ray cast naturally.
