...
+
NOTE: Other navigation methods are available, like `TCastleWalkNavigation` to walk or fly. You can also just control the camera completely through your own code, not relying on any built-in `TCastleNavigation` descendant. For example, you can process the keyboard input in your state to move/rotate the camera.
...
. Declare `WalkNavigation1: TCastleWalkNavigation;` inside the `published` section of the `TStateMain`, under the comment _{ Components designed using CGE editor.... }_.
