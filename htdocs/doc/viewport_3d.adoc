...
+
If you already have some 3D assets (see link:model_formats[3D assets formats we support]) you can of course use at this point already.
...
NOTE: As outlined in link:physics[], the `PreciseCollisions` property will at some point in the future be deprecated. What we will recommend instead is that you add a cgeref:TCastleMeshCollider[] to your scene. This waits for cgeref:TCastleWalkNavigation[] to be able to use physics to move.
