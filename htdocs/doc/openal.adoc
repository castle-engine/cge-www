...
Windows:: OpenAL libraries (DLL files) are automatically places alongside the compiled application by our link:build_tool[build tool]. You don't need to do anything, just compile and run your application using link:editor[Castle Game Engine editor] or link:build_tool[build tool].
...
## Command-line options that control the sound output
...
+
Particular applications may also expose this choice using a nice GUI, e.g. link:castle_game[The Castle] has a simple menu to choose the device.
