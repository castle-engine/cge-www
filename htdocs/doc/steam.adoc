...
+
See the https://github.com/castle-engine/castle-engine/blob/castle-steam/examples/steam/code/gamesteam.pas[examples/steam/code/gamesteam.pas] for a working example.
...
----
+
We recommend to do this along with the rest of your game initialization, in the callback you provided to the `Application.OnInitialize`. See https://github.com/castle-engine/castle-engine/blob/castle-steam/examples/steam/code/gameinitialize.pas[examples/steam/code/gameinitialize.pas]. But in principle, you can create it at any time.
...
. If cgeref:TCastleSteam.Enabled[] is `true`, then some time after you should see the cgeref:TCastleSteam.Initialized[] change from `false` to `true`. You can observe this property e.g. in the `Update` method of your view or assign a callback to cgeref:TCastleSteam.OnInitialize[].

. Once cgeref:TCastleSteam.Initialized[] is `true`, you're good to go. You can report achievements using cgeref:TCastleSteam.SetAchievement[].

== Steam application id in our example application

The example on https://github.com/castle-engine/castle-engine/tree/castle-steam/examples/steam[examples/steam] by default uses application id of https://partner.steamgames.com/doc/sdk/api/example[SteamWorks game example: SpaceWar].

NOTE: Using this example will add this game to your Steam library. If you run it for the 1st time, you may not see the achievements -- just close and run the 2nd time. This seems to be just a quirk of this test game and the way Steam automatically adds it to your library. Things are smooth for a real game that you own before you run.
...
1. `AppId` constant in the https://github.com/castle-engine/castle-engine/blob/castle-steam/examples/steam/code/gamesteam.pas[examples/steam/code/gamesteam.pas] unit.
