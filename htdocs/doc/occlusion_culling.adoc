...
* Display FPS (see https://castle-engine.io/optimization[manual about FPS])
...
* The algorithm is incompatible with doing cgeref:TCastleViewport.DynamicBatching[], so we just disable _dynamic batching_ optimization on scenes where the occlusion query is active. Why?
