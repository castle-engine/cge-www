...
- To read / write text files from an URL, use cgeref:TTextReader[] and cgeref:TTextWriter[].
...
_On Android_, you should use the https://github.com/castle-engine/castle-engine/blob/master/tools/build-tool/data/android/integrated-services/read_external_storage/README.md[read_external_storage service] to be able to read storage files (e.g. from SD card) through the `file` protocol.
...
_On Android_, you should use the https://github.com/castle-engine/castle-engine/blob/master/tools/build-tool/data/android/integrated-services/download_urls/README.md[download_urls service] to support `http` and `https` protocols.
