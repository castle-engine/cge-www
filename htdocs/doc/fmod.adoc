...
One of the alternative backends is https://www.fmod.com/[FMOD], using a commercial (closed-source) FMOD library. FMOD works nicely, and is even more cross-platform (in particular it supports also link:nintendo_switch[Nintendo Switch]). It also supports more link:sound#formats[sound file formats] out-of-the-box, like _MP3_ and _FLAC_ (in addition to our guaranteed formats _WAV_ and _OggVorbis_).
