...
== Special considerations for Delphi on Linux

=== Data files deployment (for all non-Windows platforms with Delphi)
...
=== Replacing Application.Run in FireMonkey applications

Due to technical limitations of FMXLinux, you cannot just use Application.Run in FMX (FireMonkey) applications that you want to run on Linux (if you want to have smoothly working TCastleControl). You need to replace it with

```
TCastleControl.ApplicationRun
```

call. Add to the uses clause the `Fmx.CastleControl` unit where it is defined.
