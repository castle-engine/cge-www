...
The default target OS/CPU corresponds to your current (source) OS/CPU, which is pretty much guaranteed to be `Win64` since Delphi runs on Windows and all Windowses are now 64-bit (it doesn't matter that Delphi IDE is 32-bit here). So this is equivalent to
