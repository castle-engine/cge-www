...
# Also, beware if you have Python > 3.12 see below.
...
[NOTE]
====
If you get error from PAServer about `_Py_IsFinalizing` missing when trying to debug the Linux application, it means that your Linux distribution has newer Python (> 3.12, while Embarcadero PAServer requires \<= 3.12).

This happens in particular on _Ubuntu 26.04_ with Delphi 13.1.

You have to install older Python, which can be done side-by-side using https://launchpad.net/~deadsnakes/+archive/ubuntu/ppa[Python packages in PPA by "deadsnakes"] like this:

```
sudo add-apt-repository ppa:deadsnakes/ppa
sudo apt update
sudo apt install python3.11-dev
ls /usr/lib/x86_64-linux-gnu/libpython3* # should show new Python versions
ln -s /usr/lib/x86_64-linux-gnu/libpython3.11.so.1 libpython3.so
```
====
...
- Examples using cgeref:TCastleWindow[], our recommended approach to develop games with _Castle Game Engine_, where the engine controls the entire application window. Most CGE examples use it, test e.g. https://github.com/castle-engine/castle-engine/tree/master/examples/3d_games/eye_of_beholder[examples/3d_games/eye_of_beholder], https://github.com/castle-engine/castle-engine/tree/master/examples/2d_games/platformer[examples/2d_games/platformer].
...
For Linux, this is now equally simple:

```
castle-engine compile --compiler=delphi --verbose --os=linux --cpu=x86_64
```

NOTE: In the past, this was more complicated, because https://github.com/castle-engine/castle-engine/blob/master/tools/build-tool/delphi_dcc_vs_msbuild.md[Delphi DCC required a bunch of extra parameters] to be able to compile for Linux. We now use `msbuild` for command-line building with Delphi, and things are simple.

WARNING: Using the `msbuild` can cause errors with 32000 command-line length limit, documented at https://github.com/castle-engine/castle-engine/blob/master/tools/build-tool/delphi_dcc_vs_msbuild.md[dcc vs msbuild]. If you experience this, try to move the engine installation to a shorter path.
