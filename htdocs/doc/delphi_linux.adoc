...
Due to technical limitations, you cannot just use `Application.Run` in FMX (FireMonkey) applications that you want to run on Linux (if you want to have smoothly working cgeref:TCastleControl[]). You need to replace it (usually in the main program file) with
