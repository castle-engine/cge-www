...
Unfortunately, Delphi's `TNetHttpClient` component fails to download larger files on Linux. This is reproducible without CGE, using our test programs in https://github.com/michaliskambi/delphi-http-download :
...
Use any of the above examples, change URL to https://castle-engine.io/latest.zip -- it will fail with a timeout. Tested with both Delphi 11.3 and 12.

In effect, our cgeref:TCastleDownload[] component also fails in this case. cgeref:TCastleDownload[] on Delphi/Linux is just a wrapper around `TNetHttpClient`.

*Smaller requests (`GET`, `POST`) work OK*, as you can test by https://github.com/castle-engine/castle-engine/tree/master/examples/network/remote_logging[examples/network/remote_logging] and https://github.com/castle-engine/castle-engine/tree/master/examples/network/asynchronous_download[examples/network/asynchronous_download]. The latter (`asynchronous_download`) downloads OK smaller things, like https://en.wikipedia.org/wiki/Main_Page or https://castle-engine.io/modern_pascal_introduction.html . But it fails to download a big file https://castle-engine.io/latest.zip .
