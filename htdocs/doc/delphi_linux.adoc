...
NOTE: It is OK to do this (replace `Application.Run` -> `TCastleControl.ApplicationRun`, add `TCastleControl.ProcessTasks` after `Application.ProcessMessages`) even if your application does not use cgeref:TCastleControl[], or it doesn't use it on all forms / all the time. It will just work, i.e. `TCastleControl.ApplicationRun` will be a drop-in replacement for `Application.Run` on all platforms.