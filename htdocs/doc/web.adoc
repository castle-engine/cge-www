...
- link:web-demos/examples/component_gallery[User interface components] (https://github.com/castle-engine/castle-engine/tree/master/examples/component_gallery[Source code])
...
- Fonts: We have a hacky solution right now. We generate embedded font, just like when using `texture-font-to-pascal`, for all fonts found in your data. Then we compile them in.
+
The way it currently works was easy to do, but it's not the final solution, TODO: it has a few drawbacks.
+
--
- We move away from "compiling things into Pascal", because some approaches to this (`image-to-pascal`, `texture-font-to-pascal`) caused only maintenance burden, do not scale (e.g. to distance field fonts) and do not have real benefits. In practice, we want to read "external data" and we can do this on all platforms. (And if we ever need to, we could use generic FPC resources.) `texture-font-to-pascal` is deprecated for this reasons. What we really want is to convert the ttf/otf/woff to our font format (still in `data/`) that no longer needs FreeType to open at runtime. This would be similar to e.g. how we generate GPU-compressed textures.

- A practical problem with current approach is also that we embed arbitrary size (20), anti-aliasing and character set (only ASCII) for this font.

- Another practical problem is that it doesn't work on Windows, where build tool cannot use FreeType, to avoid another Windows issue (mistakenly locking the DLL you want to replace).
--
...
- The way we embed fonts has a few drawbacks. We assume hardcoded size (20), character set (ASCII) and it cannot work on Windows (because build tool right now doesn't use dynamic libraries, to avoid conflicting with removal/updating of DLLs of the project).