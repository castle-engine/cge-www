...
WARNING: This feature is in development now, only available in the https://github.com/castle-engine/castle-engine/tree/new-pointer-lock[new-pointer-lock] branch.
...
- **User can always cancel pointer lock** by pressing _Escape_. You cannot avoid this cancelling (it happens at the browser level, not in our engine). You can only react to it -- by handling cgeref:TCastleAbstractPointerLock.OnUserCancelled[Container.PointerLock.OnUserCancelled] and/or by watching the cgeref:TCastleWalkNavigation.MouseLook[] and cgeref:TCastleAbstractPointerLock.Active[Container.PointerLock.Active], which automatically turn from `true` to `false`.
+
...
begin
  inherited;
  Container.PointerLock.OnUserCancelled.Add(
    {$ifdef FPC}@{$endif} PointerLockUserCancelled);
end;

procedure TGameView.Stop;
begin
  Container.PointerLock.OnUserCancelled.Remove(
    {$ifdef FPC}@{$endif} PointerLockUserCancelled);
  inherited;
end;
