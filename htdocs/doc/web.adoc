...
[WARNING]
====
Since 2025-02-27, the fpcupdeluxe will fail to build FPC+WebAssembly because the OS name changed in FPC: `wasi` -> `wasip1`. We submitted an https://github.com/LongDirtyAnimAlf/fpcupdeluxe/issues/747[issue about it] and https://github.com/LongDirtyAnimAlf/fpcupdeluxe/pull/748[PR that fixes it], they wait to be applied. For now, get fpcupdeluxe from the fork indicated in this PR, that is: https://github.com/michaliskambi/fpcupdeluxe .

So, practically speaking, first you need to get and compile https://github.com/michaliskambi/fpcupdeluxe[this version of FpcUpDeluxe] and then use it to build and install latest ("trunk") FPC with WebAssembly support.
====
+
