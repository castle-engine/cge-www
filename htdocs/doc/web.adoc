...
- FPC includes a unit `Job_Web` with `TJSWebGL2RenderingContext`. Michalis already learned how to use it. See wiki, https://wiki.freepascal.org/WebAssembly/DOM . **We already use this approach with success, we have `CastleInternalJobWeb` exposing working WebGL API for WebAssembly.**
