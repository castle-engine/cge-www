...
+
If you use _pointer lock_ (by cgeref:TCastleWalkNavigation.MouseLook[] or cgeref:TCastleAbstractPointerLock[Container.PointerLock]), be aware that pressing _Escape_ will cause the browser to cancel the pointer lock mode. You cannot avoid this on your application side, user has always the possibility to cancel pointer lock by pressing _Escape_. You can only react to it -- by handling cgeref:TCastleAbstractPointerLock.OnUserCancelled[Container.PointerLock.OnUserCancelled] and/or by watching the cgeref:TCastleWalkNavigation.MouseLook[] and cgeref:TCastleAbstractPointerLock.Active[Container.PointerLock.Active], which automatically turn to `false`.
+
For example, a reasonable thing to do in an FPS game is to pause the game and show options menu when user cancels pointer lock. User should click on something like _"Resume Game"_ to resume game, reentering the pointer lock mode.
+
WARNING: You can request pointer lock / mouse look from many places, like `Press` or `Start` or `Update` view methods (as long as you satisfy the _transient activation_ requirement, see below). But you should not try to set `WalkNavigation.MouseLook:=true` unconditionally in `Update` without reacting to cgeref:TCastleAbstractPointerLock.OnUserCancelled[Container.PointerLock.OnUserCancelled]. This would be bad for user experience: if user wants to cancel pointer lock (possibly to use mouse for something else, like clicking on the browser UI), let them, instead of reactivating the pointer lock immediately.
+
Basically, if your game relies on the _mouse look (or more generic pointer lock)_, make sure to 1. honor when user cancels it (don't try to force reactivating pointer lock immediately), and 2. give user a way to reenter pointer lock mode by some explicit action (like _"Resume Game"_).
+
Example:
+
```pascal
...
begin
  inherited;
  { Called when user resumed the game, e.g. by clicking on "Resume Game"
    button in options menu.
    This was *possibly* called after we entered options from
    PointerLockUserCancelled, after WalkNavigation.MouseLook was set to false
    by user cancelling. }
