...
1. link:web-demos/examples/web/simplest_viewport[3D viewport, with animated light, dropping boxes with physics].
+
...
2. link:web-demos/examples/web/simplest_invaders[2D "Invaders" game].
+
...
3. link:web-demos/examples/web/simplest[The first, simplest example that runs on the web]!
+
...
- TODO: PBR shaders (used by most 3D demos) have weird error at compilation on WebGL.
...
2. Enter any CGE project https://github.com/castle-engine/castle-engine/blob/master/examples/web/upload.sh#L72[that was tested for web] and compile and run it for web.
