...
- link:web-demos/tests[Automatic Engine Tests] (https://github.com/castle-engine/castle-engine/tree/master/tests[Source code]). Our full, cross-platform test suite with over 470 tests, running on the web. TODO: Not all the tests execute correctly yet.
...
+
This limitation is similar to Android, iOS, and link:nintendo_switch[Nintendo Switch] limitation. You can query cgeref:TCastleApplication.MultipleWindowsPossible[] at run-time to write a cross-platform code that adjusts to this.
+
...
- WebAssembly does not support _catching_ exceptions. So anytime you `raise` an exception in Pascal, you just break the application, unconditionally.
+
...
+
However, be aware of this limitation in your own code. Do not depend on `try .. except` for the logic that should work in "production".
