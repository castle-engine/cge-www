...
+
NOTE: TODO: For now we also use the WebGL API from Pas2js, using the `WebGL` unit in Pas2js, to initialize the context. We use this now, though in the future we'll likely eliminate it -- in the end, rendering has to be done from WebAssembly anyway, so it should do 100% of the WebGL talking.
+
...
+
Our Pas2js code already has basic context initialization + render + animate code, to test it works.

- FPC includes a unit `Job_Web` with `TJSWebGL2RenderingContext`. Michalis already learned how to use it. See wiki, https://wiki.freepascal.org/WebAssembly/DOM . **We already use this approach with success, we have `CastleJobWeb` exposing working WebGL API for WebAssembly.**

- TODO: Next steps: integrate WebGL usage through CGE code, utilizing the fact that it's similar (though not identical) to OpenGL ES 2.0.

- Maybe: Define `CastleWebGL` unit, exposing these calls in a flat way, to be similar to OpenGLES, and add macro `CastleGLES:=CastleWebGL` in `castleconf.conf`. This is to be decided, how much we want to "pretend that WebGL is OpenGLES".
