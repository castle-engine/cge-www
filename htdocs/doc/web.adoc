...
The resulting application works in any modern web browser and does not require user to install anything special (like old plugins). Everything we need is already built-in in all modern web browsers, in particular _WebAssembly_ and _WebGL_ support in completely standard. _Firefox_, _Chrome_ and derivatives, are all good choices.

////
TODO: For now, on mobile (Android, Firefox) we fail:
"ReferenceError: caches is not defined"

Desktop as well as mobile browsers are supported (we support touches in a way consisten with mouse events, link:touch_input[just like on non-web mobile targets]).
////
...
Open link:web-demos/simplest[the first, simplest example that runs on the web]!
...
WARNING: Visit the above on a desktop browser. On Android, both Firefox and Chrome fail with `ReferenceError: caches is not defined`. It's a TODO to investigate and fix this.
...
- Research and fix mobile browsers, for now both Firefox and Chrome on Android fail with `ReferenceError: caches is not defined`.
