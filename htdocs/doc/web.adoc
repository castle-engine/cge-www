...
WARNING: This feature is in development now, only available in the https://github.com/castle-engine/castle-engine/tree/new-pointer-lock[new-pointer-lock] branch. It is also a _bit_ buggy in Firefox (spike on activation, but otherwise works) and _very_ buggy in Vivaldi. More testing and work on this will follow.
...
```
+
NOTE: You can also read https://github.com/castle-engine/castle-engine/blob/new-pointer-lock/examples/3d_games/explore_impressive_castle/resuming_mouse_look.md[longer explanation why `Resume` method should do `WalkNavigation.MouseLook := true`].
...
- cgeref:TCastleApplication.ProcessMessage[Application.ProcessMessage] cannot work on the web, as the browser must control the main loop. This is similar to the link:ios#_known_problems[iOS known issue].
+
So: do not use cgeref:TCastleApplication.ProcessMessage[Application.ProcessMessage] in a code you want to run on the web.
+
Moreover: do not use cgeref:CastleMessages[] unit, with routines like cgeref:MessageYesNo[] or cgeref:MessageOK[], since it relies on cgeref:TCastleApplication.ProcessMessage[Application.ProcessMessage].
+
