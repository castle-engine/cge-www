...
1. link:web-demos/simplest_viewport[3D viewport, with animated light, dropping boxes with physics].
+
Source code in https://github.com/castle-engine/castle-engine/tree/master/examples/web/simplest_viewport[examples/web/simplest_viewport].

2. link:web-demos/simplest_invaders[2D "Invaders" game].
+
Source code in https://github.com/castle-engine/castle-engine/tree/master/examples/web/simplest_invaders[examples/web/simplest_invaders].

3. link:web-demos/simplest[The first, simplest example that runs on the web]!
+
Source code in https://github.com/castle-engine/castle-engine/tree/master/examples/web/simplest[examples/web/simplest].
...
- Mobile web browsers are supported as well. We support touches in a way consisten with mouse events, link:touch_input[just like on non-web mobile targets]. We tested with both _Firefox_ and _Google Chrome_ on _Android_.
+
...
+
NOTE: FPC ships an example https://gitlab.com/freepascal.org/fpc/source/-/blob/main/packages/wasm-job/examples/job_web.pas?ref_type=heads[job_web.pas] that already contains everything we need, but it's a huge unit. It has ~184 thousands of lines and compiling it is slow (this is esp. problematic as current FPC 3.3.1 recompiles units more often than it should -- it seems that every change even to implementation causes rebuild of everything derived, as if we changed interface). That's why we generate our own unit, https://github.com/castle-engine/castle-engine/blob/master/src/base_rendering/web/castleinternaljobweb.pas[CastleInternalJobWeb] (merely ~19 thousands of lines) that contains only what we need.
+
...
+
So far, file-size tests are promising. Compilation of https://github.com/castle-engine/castle-engine/tree/master/examples/platformer[examples/platformer], which practically uses 100% of CGE units, yields a binary `platformer.wasm` that has 16 MB. Gzipped it has 3.4 MB.
+
...
Right now, https://github.com/castle-engine/castle-engine/tree/master[master] branch of CGE contains the work on WebAssembly. To test it:

1. Get the https://github.com/castle-engine/castle-engine/tree/master[master] branch of CGE.
...
----
+
TODO: Naturally we plan to allow compiling *any* project from `examples/` to the web. But for now, only projects in `examples/web/` are prepared for this.

[WARNING]
====
Beware of the cache!

Browsers may cache the contents of everything (HTML, JS, WASM files, application data...). We counter this somewhat (by randomizing the URL by which we access the WASM file), but still cache affects us, as other files are cached. Just refresh using _Ctrl + Shift + R_ if it seems like you look at an old application version (or even a different application that was run previously from the same server).
...
See https://github.com/castle-engine/castle-engine/blob/master/examples/web/simplest_invaders/CastleEngineManifest.xml[examples/web/simplest_invaders/CastleEngineManifest.xml] for a full example.
