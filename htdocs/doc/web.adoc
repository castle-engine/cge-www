...
- The demos use only 3D / 2D things set up from code (because they don't load data files, not even designs) so they don't look impressive.
...
+
TODO: Downloading the zip is in progress now. We have a bit of data hardcoded now.
