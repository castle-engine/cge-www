...
[WARNING]
====
This feature is in development now. This page is updated often as we go. Stay tuned!
====
...
- The resulting application works in any modern web browser and does not require user to install anything special (like old plugins). Everything we need is already built-in in all modern web browsers, in particular _WebAssembly_ and _WebGL_ support in completely standard.
...
+
WARNING: To build application web working on mobile, one needs to apply a 1-line hack to `pas2js` sources, see below for details.
...
- Input: We handle pointer events (down, motion, up) thus we handle mouse and touch input.
