...
+
This has been fixed on the FPC side, see https://wiki.freepascal.org/WebAssembly/Exceptions[FPC + WebAssembly exceptions support] for details. Our `CanCatchExceptions` for now remains `false` just to be conservative, but we plan to change it to `true` when `FPC_WASM_BRANCHFUL_EXCEPTIONS` / `FPC_WASM_EXNREF_EXCEPTIONS` / `FPC_WASM_LEGACY_EXCEPTIONS` are defined (one of modes when exceptions are catchable), once we test it. In the meantime, we encourage you to use latest FPC and test exception catching on the web.
