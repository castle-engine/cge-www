...
+
We considered just serializing cgeref:TCastleFont[], but it's not optimal, as it contains not only `OptimalSize` but also `Size` (which matters for display, but not data). It also lacks glyphs and font metadata like family name.
+
Existing `TTextureFontData` is close to what we need, this is exactly the information we need. Remake it to be `TCastleFontData`, a `TComponent` descendant, with consistent property names (`Size` -> `OptimalSize`). Save it as `xxx.castle-font-data`.
- Make a dedicated editor for `TCastleFontData` that, when saving, also saves font texture (distance field or not) alongside it and glyphs (as children components). A bit like sprite sheet editor saves atlas.
