...
- Requesting _pointer lock_, which happens when you activate cgeref:TCastleWalkNavigation.MouseLook[] or cgeref:TCastleAbstractPointerLock[Container.PointerLock], can only be done in the https://developer.mozilla.org/en-US/docs/Glossary/Transient_activation[transient activation] state, which means it must be triggered by a user interaction (like a mouse click or key press) or right after it.
+
It's safest to request it from `TMyView.Press` when user clicks on the canvas:
+
```pascal
...
```
+
It's also OK to request it from e.g. `TMyView.Update` when you do it _right after user interaction_, e.g.
+
```pascal
