...
- Tab (switches focus)
- Escape (exits fullscreen, cancels pointer lock)
--
+
...
- Special advise for handling _Escape_ key in FPS games:
+
...
- Requesting https://developer.mozilla.org/en-US/docs/Web/API/Pointer_Lock_API[pointer lock], which happens when you activate cgeref:TCastleWalkNavigation.MouseLook[] or cgeref:TCastleAbstractPointerLock[Container.PointerLock], can only be done in the https://developer.mozilla.org/en-US/docs/Glossary/Transient_activation[transient activation] state, which means it must be triggered by a user interaction (like a mouse click or key press) or right after it.
+
