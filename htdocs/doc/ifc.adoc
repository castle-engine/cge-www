...
- cgeref:TIfcProduct.SetRelativePlacement[] (move products)
...
////
TODO: This section talks about "only loading" for now, because that's what is implemented.
But we have below working about saving too, which is planned.
////
//## Loading and saving IFC in TCastleScene
## Loading IFC in TCastleScene
...
You can also use the cgeref:LoadNode[]
//TODO: and cgeref:SaveNode[] methods to load and save
method to load
cgeref:TX3DRootNode[] instance, without the need for a full cgeref:TCastleScene[]. This makes sense if you don't need to display the model at all.
