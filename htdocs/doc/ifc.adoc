...
We can load and save the IFC entities listed in API docs of cgeref:CastleIfc[] unit. This includes over 170 classes reflecting the IFC specification.
...
NOTE: Want the below features to happen? Please contact us and link:donate[donate toward the IFC support, we provide commercial support for companies too].

Easier:

- Ability to toggle which representation (3D model, 2D plan) is displayed. We have the necessary information (which object comes from what representation).

- Optimize updates more.
...
- NURBS in IFC (we already have NURBS in X3D nodes).
