...
- cgeref:TIfcSpatialElement.AddContainedElement[]
- cgeref:TIfcProject.SetupUnits[]

The goal of these utility methods is to easily construct IFC models using the most common IFC concepts (products, representations, placements). They are not strictly necessary to build/modify IFC content, but they make some operations simpler. See lower on this page for example usage of these methods to easily add walls and floors.
