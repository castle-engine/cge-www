...
We also define some utility methods to operate on IFC classes:

- cgeref:TIfcProduct.AddMeshRepresentation[]
- cgeref:TIfcProduct.AddBoxRepresentation[]
- cgeref:TIfcProduct.SetRelativePlacement[]
- cgeref:TIfcObjectDefinition.AddIsDecomposedBy[]
- cgeref:TIfcProject.SetupUnits[]

The goal of these utility methods is to easily construct IFC models using the most common IFC concepts (products, representations, placements). They simplify some IFC complexity. These methods don't do anything that you couldn't do yourself by operating on IFC classes directly. See lower on this page for example usage of these methods to easily add walls and floors.
